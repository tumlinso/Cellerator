...
csd::blocked_ell_view make_device_view(const cellerator::matrix::blocked_ell *part,
                                       const csd::partition_record<cellerator::matrix::blocked_ell> &record) {
...
csd::sliced_ell_view make_device_view(const cellerator::matrix::sliced_ell *part,
                                      const csd::partition_record<cellerator::matrix::sliced_ell> &record) {
...
PreprocessSession preprocess_cellshard(const std::string &path, const PreprocessOptions &options) {
    ...
    if (summary.matrix_format == "blocked_ell") {
        return run_dataset_layout<cellerator::matrix::blocked_ell>(path, "blocked_ell", summary, options, masks);
    }
    if (summary.matrix_format == "sliced_ell") {
        return run_dataset_layout<cellerator::matrix::sliced_ell>(path, "sliced_ell", summary, options, masks);
    }
