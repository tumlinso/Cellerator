...
- `cellpack-bp06-11-parallel-execution` | status: in_progress | execution: claimed | owner: coordination | file: `todos/cellpack-bp06-11-parallel-execution.md` | next: `CP08_DEVICE_READY` and `CP09_REFERENCE_READY` are published and idle; appoint the Barrier D integrator and do not open Phase E.
- `cellpack-data-inferred-block-packing-roadmap` | status: in_progress | execution: claimed | owner: coordination | file: `todos/cellpack-data-inferred-block-packing-roadmap.md` | next: Do not implement from this parent. Both Phase D gates are idle; Barrier D integration is the sole next action.
...
- `cellpack-bp08-warp-tiles` | status: in_progress | execution: idle | owner: unassigned | file: `todos/cellpack-bp08-warp-tiles.md` | next: `CP08_DEVICE_READY` is published; remain idle until Barrier D validates, commits, pushes, and closes CP-BP-08.
- `cellpack-bp09-native-runtime-consumers` | status: in_progress | execution: idle | owner: unassigned | file: `todos/cellpack-bp09-native-runtime-consumers.md` | next: `CP09_REFERENCE_READY` is published; remain idle until `CP08_DEVICE_READY` and Barrier D open the Phase E direct CUDA consumer.
