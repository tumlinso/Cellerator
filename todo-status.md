...
- `cellpack-bp06-11-parallel-execution` | status: in_progress | execution: claimed | owner: coordination | file: `todos/cellpack-bp06-11-parallel-execution.md` | next: Phase C exact disjoint leases are published; await explicit CP-BP-08/11 assignments from the same current pushed base, then stop both at their host/held-out gates for Barrier C.
- `cellpack-data-inferred-block-packing-roadmap` | status: in_progress | execution: claimed | owner: coordination | file: `todos/cellpack-data-inferred-block-packing-roadmap.md` | next: Do not implement from this parent. Phase C CP-BP-08/11 are the current unclaimed parallel frontier.
...
- `cellpack-bp08-warp-tiles` | status: planned | execution: ready | owner: unassigned | file: `todos/cellpack-bp08-warp-tiles.md` | next: Await assignment; then claim only the exact Phase C host ABI/reference lease, publish `CP08_HOST_ABI_READY`, release, and stop before CUDA Phase D.
...
- `cellpack-bp11-statistical-validation` | status: in_progress | execution: idle | owner: unassigned | file: `todos/cellpack-bp11-statistical-validation.md` | next: Await assignment; then claim only the exact Phase C frozen-plan/record-held-out adapter lease, publish `CP11_HELDOUT_READY`, release, and stop.
