...
namespace cellerator::compute::sparse::ops::base {
...
void csr_row_scale_fwd_f16_f32(
    ...
    std::uint32_t rows,
    float *out_values) {
    runtime::cuda_require(cudaSetDevice(ctx.device), "cudaSetDevice(csr_row_scale_fwd)");
    if (rows == 0) return;
    ...
    runtime::cuda_require(cudaGetLastError(), "csr_row_scale_fwd_kernel");
}
...
void csr_row_scale_bwd_values_f16_f32(
    ...
    std::uint32_t rows,
    float *grad_values) {
    runtime::cuda_require(cudaSetDevice(ctx.device), "cudaSetDevice(csr_row_scale_bwd_values)");
    if (rows == 0) return;
    const int blocks = static_cast<int>((static_cast<std::size_t>(rows) + kRowThreads - 1u) / kRowThreads);
    csr_row_scale_bwd_values_kernel_<<<blocks, kRowThreads, 0, ctx.stream>>>(major_ptr, grad_out, row_scales, rows, grad_values);
    runtime::cuda_require(cudaGetLastError(), "csr_row_scale_bwd_values_kernel");
}
...
void csr_row_scale_bwd_scales_f16_f32(
    ...
    std::uint32_t rows,
    float *grad_scales) {
    runtime::cuda_require(cudaSetDevice(ctx.device), "cudaSetDevice(csr_row_scale_bwd_scales)");
    if (rows == 0) return;
    const int blocks = static_cast<int>((static_cast<std::size_t>(rows) + kRowThreads - 1u) / kRowThreads);
    csr_row_scale_bwd_scales_kernel_<<<blocks, kRowThreads, 0, ctx.stream>>>(major_ptr, values, grad_out, rows, grad_scales);
    runtime::cuda_require(cudaGetLastError(), "csr_row_scale_bwd_scales_kernel");
}
...
void sparse_value_sum_f16_f32(
    ...
    std::uint32_t nnz,
    float *out_scalar) {
    runtime::cuda_require(cudaSetDevice(ctx.device), "cudaSetDevice(sparse_value_sum)");
    if (nnz == 0) {
        const float zero = 0.0f;
        runtime::cuda_require(cudaMemcpyAsync(out_scalar, &zero, sizeof(float), cudaMemcpyHostToDevice, ctx.stream), "cudaMemcpyAsync(sparse_value_sum zero)");
        return;
    }
    ...
    void *scratch = runtime::request_scratch(arena, cub_offset + cub_bytes);
    ...
    runtime::cuda_require(cudaGetLastError(), "widen_half_to_float_kernel");
...
void sparse_value_sum_bwd_fill_f32(
    ...
    std::uint32_t nnz,
    float *grad_values) {
    runtime::cuda_require(cudaSetDevice(ctx.device), "cudaSetDevice(sparse_value_sum_bwd_fill)");
    if (nnz == 0) return;
    const int blocks = static_cast<int>((static_cast<std::size_t>(nnz) + kValueThreads - 1u) / kValueThreads);
    fill_from_scalar_kernel_<<<blocks, kValueThreads, 0, ctx.stream>>>(grad_scalar, nnz, grad_values);
    runtime::cuda_require(cudaGetLastError(), "fill_from_scalar_kernel");
}
...
void csr_spmv_fwd_f16_f32(
    ...
    const float *vector,
    float *out) {
    runtime::cuda_require(cudaSetDevice(ctx.device), "cudaSetDevice(csr_spmv_fwd)");
    if (rows == 0) return;
    const int blocks = static_cast<int>((static_cast<std::size_t>(rows) + kRowThreads - 1u) / kRowThreads);
    csr_spmv_fwd_kernel_<<<blocks, kRowThreads, 0, ctx.stream>>>(major_ptr, minor_idx, values, rows, vector, out);
    runtime::cuda_require(cudaGetLastError(), "csr_spmv_fwd_kernel");
}
...
void csr_spmv_bwd_values_f16_f32(
    ...
    std::uint32_t rows,
    float *grad_values) {
    runtime::cuda_require(cudaSetDevice(ctx.device), "cudaSetDevice(csr_spmv_bwd_values)");
    if (rows == 0) return;
    const int blocks = static_cast<int>((static_cast<std::size_t>(rows) + kRowThreads - 1u) / kRowThreads);
    csr_spmv_bwd_values_kernel_<<<blocks, kRowThreads, 0, ctx.stream>>>(major_ptr, minor_idx, grad_out, vector, rows, grad_values);
    runtime::cuda_require(cudaGetLastError(), "csr_spmv_bwd_values_kernel");
}
...
void csr_spmv_bwd_vector_f16_f32(
    ...
    std::uint32_t cols,
    float *grad_vector) {
    runtime::cuda_require(cudaSetDevice(ctx.device), "cudaSetDevice(csr_spmv_bwd_vector)");
    if (rows == 0 || cols == 0) return;
    runtime::cuda_require(cudaMemsetAsync(grad_vector, 0, static_cast<std::size_t>(cols) * sizeof(float), ctx.stream), "cudaMemsetAsync(csr_spmv_bwd_vector)");
    const int blocks = static_cast<int>((static_cast<std::size_t>(rows) + kRowThreads - 1u) / kRowThreads);
    csr_spmv_bwd_vector_kernel_<<<blocks, kRowThreads, 0, ctx.stream>>>(major_ptr, minor_idx, values, grad_out, rows, grad_vector);
    runtime::cuda_require(cudaGetLastError(), "csr_spmv_bwd_vector_kernel");
}
...
void csr_feature_affine_quantize_fwd_bwd_f16_f32(
    ...
    float *grad_offset) {
    runtime::cuda_require(cudaSetDevice(ctx.device), "cudaSetDevice(csr_feature_affine_quantize)");
    if (cols == 0u) return;
    ...
    runtime::cuda_require(cudaMemsetAsync(reconstruction_loss, 0, sizeof(float), ctx.stream), "cudaMemsetAsync(csr_feature_affine_quantize reconstruction)");
    runtime::cuda_require(cudaMemsetAsync(range_loss, 0, sizeof(float), ctx.stream), "cudaMemsetAsync(csr_feature_affine_quantize range)");
    ...
    feature_affine_quantize_baseline_kernel_<<<feature_blocks, kFeatureThreads, 0, ctx.stream>>>(
        ...
        grad_log_scale,
        grad_offset);
    runtime::cuda_require(cudaGetLastError(), "feature_affine_quantize_baseline_kernel(csr)");
    ...
    csr_feature_affine_quantize_correction_kernel_<<<row_blocks, kRowThreads, 0, ctx.stream>>>(
        ...
        grad_log_scale,
        grad_offset);
    runtime::cuda_require(cudaGetLastError(), "csr_feature_affine_quantize_correction_kernel");
}
...
void blocked_ell_feature_affine_quantize_fwd_bwd_f16_f32(
    ...
    float *grad_log_scale,
    float *grad_offset) {
    runtime::cuda_require(cudaSetDevice(ctx.device), "cudaSetDevice(blocked_ell_feature_affine_quantize)");
    ...
    runtime::cuda_require(cudaMemsetAsync(reconstruction_loss, 0, sizeof(float), ctx.stream), "cudaMemsetAsync(blocked_ell_feature_affine_quantize reconstruction)");
    runtime::cuda_require(cudaMemsetAsync(range_loss, 0, sizeof(float), ctx.stream), "cudaMemsetAsync(blocked_ell_feature_affine_quantize range)");
    const float inv_elements = rows == 0u
        ? 0.0f
        : 1.0f / static_cast<float>(static_cast<double>(rows) * static_cast<double>(cols));
    const float inv_features = 1.0f / static_cast<float>(cols);
    ...
    feature_affine_quantize_baseline_kernel_<<<feature_blocks, kFeatureThreads, 0, ctx.stream>>>(
        ...
        grad_log_scale,
        grad_offset);
    runtime::cuda_require(cudaGetLastError(), "feature_affine_quantize_baseline_kernel(blocked ell)");
    ...
    blocked_ell_feature_affine_quantize_correction_kernel_<<<row_blocks, kRowThreads, 0, ctx.stream>>>(
        ...
        grad_log_scale,
        grad_offset);
    runtime::cuda_require(cudaGetLastError(), "blocked_ell_feature_affine_quantize_correction_kernel");
}
...
void csr_spmm_bwd_values_f16_f32(
    ...
    float *grad_values) {
    runtime::cuda_require(cudaSetDevice(ctx.device), "cudaSetDevice(csr_spmm_bwd_values)");
    ...
    runtime::cuda_require(cudaGetLastError(), "csr_spmm_bwd_values_kernel");
}
...
void csr_spmm_bwd_rhs_f16_f32(
    ...
    runtime::cuda_require(cudaSetDevice(ctx.device), "cudaSetDevice(csr_spmm_bwd_rhs)");
    ...
    runtime::cuda_require(cudaGetLastError(), "csr_spmm_bwd_rhs_kernel");
}
...
void csr_spmv_fwd_f32_lib(
    ...
    float *out) {
    if (rows == 0 || cols == 0) return;
    runtime::cuda_require(cudaSetDevice(ctx.device), "cudaSetDevice(csr_spmv_f32_lib)");
    ...
    std::size_t &bytes = runtime::cached_spmv_bytes(cache, CUSPARSE_OPERATION_NON_TRANSPOSE);
    if (bytes == 0) {
        cusparse_require_(
