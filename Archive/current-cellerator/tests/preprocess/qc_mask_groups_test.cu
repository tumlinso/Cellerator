...
int main() {
    ...
    cpre::qc_feature_annotation_view features{ids, names, types, modalities, 4u};
    if (!cpre::compile_default_qc_feature_group_masks(&features, nullptr, masks)) return 1;
    if (!require((masks[0] & cpre::qc_group_bit(cpre::qc_group_mt)) != 0u, "mt default missing")) return 2;
    if (!require((masks[1] & cpre::qc_group_bit(cpre::qc_group_ribo)) != 0u, "ribo default missing")) return 3;
    if (!require((masks[2] & cpre::qc_group_bit(cpre::qc_group_hb)) != 0u, "hb default missing")) return 4;

    cpre::qc_group_rule_view user_rule{3u, "custom", nullptr, "USER_GENE", nullptr, "gene", "rna"};
    if (!cpre::compile_qc_feature_group_masks(&features, &user_rule, 1u, masks, masks)) return 5;
    masks[3] |= cpre::qc_group_bit(cpre::qc_group_mt);
    ...
    cpre::init(&workspace);
    if (!cpre::setup(&workspace, 0)) return 10;
    ...
    cpre::cell_metrics_view metrics{};
    if (!cpre::compute_qc_metrics(&view, &workspace, &groups, &filter, &metrics)) return 11;
    ...
    cpre::qc_group_config_view no_groups{0u, nullptr, nullptr, nullptr};
    cpre::cell_qc_filter_params no_group_filter{1.0f, 1u, 1.0f, 0u};
    if (!cpre::compute_qc_metrics(&view, &workspace, &no_groups, &no_group_filter, &metrics)) return 24;
