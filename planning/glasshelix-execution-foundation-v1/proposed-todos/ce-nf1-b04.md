...
Explicit prerequisites: `CE-NF1-B03`. Earlier entries in the lane queue also apply.
...
1. Minimal build and regression build both configure independently.
2. Hiding or removing existing regressions to make minimal build green is not accepted.
...
Evidence kind: **executed_host_test**. Required CTest names: `ce_nf1_b04`.

```sh
python3 -B planning/glasshelix-execution-foundation-v1/scripts/run_gate.py --group b04
```
