...
Project: `cellerator`. First-class lane: `CE-NF1-L-H`. Workspace: isolated_merge.
Explicit prerequisites: `CE-NF1-H05`. Earlier entries in the lane queue also apply.
...
1. False predicates can avoid invalid branch evaluation when declared by the mathematical operation.
2. Multiply-by-zero cannot silently substitute for conditional exclusion on NaN or invalid expressions.
...
- `include/Cellerator/compute/operation/indexed_mechanism`
- `src/compute/operation/indexed_mechanism`
- `tests/native_foundation/nary`
...
Evidence kind: **executed_cuda_test**. Required CTest names: `ce_nf1_h06`.

```sh
python3 -B planning/glasshelix-execution-foundation-v1/scripts/run_gate.py --group h06
```
