...
Explicit prerequisites: `CE-NF1-H06`. Earlier entries in the lane queue also apply.
...
1. At least one truly combinatorial mechanism runs on V100 through the linked library.
2. A pairwise-only facade or unlinked registration does not satisfy this capability.
...
Evidence kind: **executed_cuda_test**. Required CTest names: `ce_nf1_h07`.

```sh
python3 -B planning/glasshelix-execution-foundation-v1/scripts/run_gate.py --group h07
```
