...
Project: `cellerator`. First-class lane: `CE-NF1-L-P`. Workspace: isolated_merge.
Explicit prerequisites: `CE-NF1-P02`, `CE-NF1-V03`. Earlier entries in the lane queue also apply.
...
1. Two serially driven instances share immutable program data without sharing mutable scratch.
2. Host serialization assumptions, device ownership and external read boundaries are enforced rather than implied.
...
- `tests/native_foundation/program`
...
Evidence kind: **executed_host_test**. Required CTest names: `ce_nf1_p03`.

```sh
python3 -B planning/glasshelix-execution-foundation-v1/scripts/run_gate.py --group p03
```
