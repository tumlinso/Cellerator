...
Explicit prerequisites: `CE-NF1-M10`. Earlier entries in the lane queue also apply.
...
1. Independent finite-difference and duality checks cover state and parameter directions.
2. Forward-only blocks reject derivative requests instead of returning zero.
...
Evidence kind: **executed_host_test**. Required CTest names: `ce_nf1_d01`.

```sh
python3 -B planning/glasshelix-execution-foundation-v1/scripts/run_gate.py --group d01
```
