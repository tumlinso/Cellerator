...
Project: `cellerator`. First-class lane: `CE-NF1-L-M`. Workspace: isolated_merge.
Explicit prerequisites: `CE-NF1-M40`, `CE-NF1-T06`, `CE-NF1-O07`. Earlier entries in the lane queue also apply.
...
1. Publish qualified capability, regression, sanitizer and complete-cost evidence for GH final integration.
2. Publish exact source, interface and evidence hashes; never reach a checkpoint on a declaration-only, skipped, stale or mock result.
...
- `CMakeLists.txt`
- `docs/glasshelix_execution_foundation_v1/integration`
...
Evidence kind: **integrated_executed_tests**. Required CTest names: `ce_nf1_retained_regressions`, `ce_nf1_sanitizer_clean`, `ce_nf1_complete_cost_receipt`.

```sh
python3 -B planning/glasshelix-execution-foundation-v1/scripts/run_gate.py --group m50
```
