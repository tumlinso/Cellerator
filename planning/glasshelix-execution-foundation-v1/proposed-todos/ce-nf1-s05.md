...
Explicit prerequisites: `CE-NF1-S04`. Earlier entries in the lane queue also apply.
...
1. Removing and restoring a relation produces expected updates and stale-ticket errors.
2. Fast value-only updates must never rebuild topology; old maps cannot address the replacement.
...
Evidence kind: **executed_cuda_test**. Required CTest names: `ce_nf1_s05`.

```sh
python3 -B planning/glasshelix-execution-foundation-v1/scripts/run_gate.py --group s05
```
