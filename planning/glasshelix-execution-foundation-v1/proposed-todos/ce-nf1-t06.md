...
Project: `cellerator`. First-class lane: `CE-NF1-L-T`. Workspace: isolated_merge.
Explicit prerequisites: `CE-NF1-T05`. Earlier entries in the lane queue also apply.
...
1. Successful receipts record exact source, binary, tool, device and zero-error summary.
2. Missing hardware, contaminated timing and expected-failure process modes cannot become passing accelerator evidence.
...
- `tests/relation_update_spine_v1`
- `tests/semantic_spine`
- `tests/execution/program_test.cu`

## Initial exclusive write scope

- `tests/native_foundation/reference`
- `tests/native_foundation/conformance`
- `docs/glasshelix_execution_foundation_v1/validation`
...
Evidence kind: **executed_cuda_test**. Required CTest names: `ce_nf1_t06`.

```sh
python3 -B planning/glasshelix-execution-foundation-v1/scripts/run_gate.py --group t06
```
