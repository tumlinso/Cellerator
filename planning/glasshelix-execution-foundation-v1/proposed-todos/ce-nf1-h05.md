...
Explicit prerequisites: `CE-NF1-H04`. Earlier entries in the lane queue also apply.
...
1. Both paths match values and preserve operation provenance across composition.
2. A custom opaque block cannot claim dependency pruning or differentiation it has not supplied.
...
Evidence kind: **executed_host_test**. Required CTest names: `ce_nf1_h05`.

```sh
python3 -B planning/glasshelix-execution-foundation-v1/scripts/run_gate.py --group h05
```
