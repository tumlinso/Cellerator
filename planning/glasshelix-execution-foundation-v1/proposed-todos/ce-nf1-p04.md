...
Explicit prerequisites: `CE-NF1-P03`. Earlier entries in the lane queue also apply.
...
1. Repeated steady-state execution has no hidden allocations or whole-state canonicalization.
2. Alias overlap and use-after-completion assumptions are tested with independent sentinels.
...
Evidence kind: **executed_host_test**. Required CTest names: `ce_nf1_p04`.

```sh
python3 -B planning/glasshelix-execution-foundation-v1/scripts/run_gate.py --group p04
```
