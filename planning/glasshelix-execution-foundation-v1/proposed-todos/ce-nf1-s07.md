...
Explicit prerequisites: `CE-NF1-S06`. Earlier entries in the lane queue also apply.
...
1. The selected path is explainable from measured benefit and requested semantics.
2. Dynamic support is not declared universally faster; unsupported derivative paths fail visibly.
...
Evidence kind: **executed_cuda_test**. Required CTest names: `ce_nf1_s07`.

```sh
python3 -B planning/glasshelix-execution-foundation-v1/scripts/run_gate.py --group s07
```
