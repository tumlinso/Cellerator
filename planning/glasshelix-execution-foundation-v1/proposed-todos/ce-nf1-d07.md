...
Explicit prerequisites: `CE-NF1-D06`. Earlier entries in the lane queue also apply.
...
1. Boundary and stale-primal tests produce explicit diagnostic status.
2. Numerical finite differences remain labeled approximation, not a silent substitute for an exact action.
...
Evidence kind: **executed_host_test**. Required CTest names: `ce_nf1_d07`.

```sh
python3 -B planning/glasshelix-execution-foundation-v1/scripts/run_gate.py --group d07
```
