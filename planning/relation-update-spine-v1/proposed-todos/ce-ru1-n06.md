# CE-RU1-N06: Finish rebinding, failure, capture and instrumentation boundaries

Lane: `CE-RU1-L-NATIVE`. Status on delivery: **planned**, not claimed or implemented.
Dependencies: `CE-RU1-N05`

## Goal and implementation direction

Make checked destruction drain only owned work and registered reader completion; refuse unreleased borrows. Keep existing destroy safe by a documented no-borrow precondition or a status-returning close path. On partial submission errors poison the pair; do not claim recovery through counter rollback. Reject full mutable capture before the first side effect while preserving N1 read-only graph replay. Report real prep, refresh, update, map/pack, sparse/WMMA/residual launches and canonicalization counts.
...
- `tests/relation_update_spine_v1/native_failure_test.cu`
...
1. New mutable operations cannot be accidentally captured into an unsupported graph.
2. Fault-injection and ownership tests cover rejected metadata, partial enqueue and asynchronous-error observation.
3. Statistics are evidence counters updated from actual branches, not projected or constant expected values.

## Required validation cases

- Stale pointer/version; busy lease close; same-device wrong stream; empty relation; capture refusal; N1 graph regression.
...
Freeing borrowed storage or converting a poisoned state to success would invalidate the whole spine.
