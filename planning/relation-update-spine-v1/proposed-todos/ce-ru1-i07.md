# CE-RU1-I07: Retire superseded on-path duplication without losing good code

Lane: `CE-RU1-L-INTEGRATE`. Status on delivery: **planned**, not claimed or implemented.
Dependencies: `CE-RU1-I06`

## Goal and implementation direction

Use the I01 caller audit and now-tested core replacement to remove duplicated forward/transpose/edge-gradient/update ownership where it is superseded. Preserve useful bias/ReLU/RMS/other algorithms in appropriate core facilities or leave their existing consumers connected; do not delete them merely because this epic does not expose them. Compatibility wrappers need not survive without a real consumer. Any additional affected path requires explicit recorded scope handoff, not a stealth repository purge.
...
1. Every moved/removed implementation has a destination or reason and preserved regression coverage.
2. No duplicate canonical semantics remain on the new path; no unrelated adapter or model functionality is silently discarded.
3. Integrated relevant tests rerun after pruning.

## Required validation cases

- Reference/caller audit, link tests of remaining consumers and full RU1 regression.
...
Legacy API removal can be justified; losing its only performant algorithm cannot.
