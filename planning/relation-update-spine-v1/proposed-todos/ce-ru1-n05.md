...
Dependencies: `CE-RU1-N04`, `CE-RU1-R02`, `CE-RU1-W04`

## Goal and implementation direction

Integrate the runtime readiness component, sparse/hybrid gradient dispatch and updates into the pair. Owner stream performs old-generation work, waits for registered reader completion, updates values, then records publication. A successful metadata transition means publication enqueued; completion is checked through event/stream observation. Initial publish and external replacement obey the same reader discipline. No cudaDeviceSynchronize or per-step allocation.
...
- `tests/relation_update_spine_v1/update_publication_test.cu`
...
1. Publication happens after all writes; newly published generation is consumed on another nonblocking stream.
2. Owner can update after reader-return enqueues completion without host-wide synchronization.
3. No false generation advancement or successful-receipt emission after rejected/failed enqueue.

## Required validation cases

- Two update styles across at least three generations; delayed reader; immediate next forward; stale-generation rejection.
...
Reusing an event does not preserve the previous generation storage contents.
