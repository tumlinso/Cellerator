# CE-RU1-F02: Extend the bounded parser and Sema bridge for the closure

Lane: `CE-RU1-L-FRONTEND`. Status on delivery: **planned**, not claimed or implemented.
Dependencies: `CE-RU1-F01`

## Goal and implementation direction

Support a deliberately bounded embedded sequence for forward, transpose, edge gradient, delta/gradient update and publication using existing parser/Sema mechanics. Loaded metadata supplies names, biological identities, widths and numeric policy, not prebuilt native operations. Preserve precise diagnostics/provenance and explicitly reject general translation units, imports and unrelated expressions. The core should not parse source strings.
...
- `tests/relation_update_spine_v1/source_bridge_test.cc`
...
1. The supplied closure source independently lowers into canonical descriptors equivalent to native construction.
2. Wrong symbols, shadowed identities, axis flips, undeclared outputs, unsupported syntax and ambiguous update order yield diagnostic failure.

## Required validation cases

- Positive demo source and multiple rejection cases; not a string-equality stub recognizing only the demo.
...
A compiler-origin flag attached to a native descriptor is not a second semantic origin.
