# CE-RU1-W03: Pack explicit half-rounded operands with reusable scratch

Lane: `CE-RU1-L-WMMA`. Status on delivery: **planned**, not claimed or implemented.
Dependencies: `CE-RU1-W02`

## Goal and implementation direction

Materialize half-rounded source and cotangent operands into aligned reusable panels only when the semantic profile permits it. Use CUDA nearest-even conversion, zero padding, independently sized strides and uint64-safe offset checks. Sparse residual consumes the same rounded values, not full-f32 leftovers. Cache pack results only against explicit operand identity/version/layout, never pointer equality. Refresh values changing at the same address. Strict-f32 requests remain outside WMMA eligibility.
...
- `tests/relation_update_spine_v1/gradient_pack_test.cu`
...
1. Pack and sparse fallback agree on the exact quantized input values and tail initialization.
2. Same-address new version forces refresh; same version may reuse only under the documented immutable-through-completion contract.
3. No hot allocation or canonical edge-value packing.

## Required validation cases

- Rounding ties, subnormal/overflow policy, reused pointers, NaN/inf propagation, poisoned padding sentinels.
...
A mixed gradient without explicit quantization semantics is not the same mathematical request.
