# CE-RU1-R02: Add bounded consumer read leases and writer join

Lane: `CE-RU1-L-READINESS`. Status on delivery: **planned**, not claimed or implemented.
Dependencies: `CE-RU1-R01`

## Goal and implementation direction

Implement one outstanding const value-read lease for v1, recyclable after end_read records consumer completion and submits owner-stream wait. This small bound permits sequential consumers on arbitrary same-device streams without adding concurrent prepared execution. The next writer rejects an unreturned lease rather than blocking the host indefinitely. Tickets carry identity, generation and nonce; duplicate return, wrong stream and old token replay fail. Do not reuse an event until its relevant waits have been submitted in the serialized protocol.
...
- `tests/relation_update_spine_v1/read_lease_test.cu`
...
1. Producer-ready and consumer-done edges are both explicit and tested without host/device-global sync.
2. A stale lease cannot release a newer reader; next mutation joins the correct completion event.
3. Pool exhaustion is explicit; the one-lease limit is documented as a bounded runtime policy, not mathematics.

## Required validation cases

- Delayed D2D reader; begin/begin rejection; stale return; return then immediate update; repeat on two distinct consumer streams.
...
Allowing unrestricted concurrent borrowing would require a larger reclamation design outside this epic.
