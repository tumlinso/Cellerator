...
Dependencies: `CE-RU1-R02`

## Goal and implementation direction

Stress repeated ready/done event reuse, stale token attempts, failed record/wait and reader return ordering. Supply deterministic adversarial scheduling with a small test-only delay kernel; do not use CUDA API calls from host callbacks or spinlocks that can deadlock occupancy. Document the API cannot protect raw pointers used after lease return and cannot prove successful asynchronous execution from enqueue alone.
...
- `tests/relation_update_spine_v1/readiness_adversarial_test.cu`
- `docs/relation_update_spine_v1/readiness.md`
...
1. Tests distinguish enqueue, ready and observed completion, including failure outcomes.
2. No use-after-free under valid leases; invalid borrower behavior is rejected where observable and documented otherwise.

## Required validation cases

- Long repeated-generation run; fault-injected event API; stream-local synchronization only at observation boundaries.
...
Sanitizer alone does not prove the synchronization protocol; dependency tests are also required.
