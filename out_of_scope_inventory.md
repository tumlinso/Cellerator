...
| Legacy Torch custom-op backend | `components/CelleraTorch/` | Torch/libtorch wrappers should not define the native Cellerator runtime when sparse operators are the Cellerator ownership point. | `components/CelleraTorch/` for Torch wrappers; native compute layers for reusable sparse math | `migration-held` | Split from native Cellerator while the broader sister-project layout is still in-tree. | Keep Torch-linked wrappers in `components/CelleraTorch/`; move any reusable sparse math discovered there into framework-independent native compute layers. |
...
- Do not treat preprocessing engines or neighbor/search engines as out of scope
  merely because their human-facing API is deferred. Cellerator may own those
  engines when they support sparse layout optimization, file/runtime payload
  construction, or native sparse execution.
