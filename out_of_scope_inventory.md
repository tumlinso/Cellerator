...
| Legacy Torch custom-op backend | `src/compute/model_ops/`, legacy entries in `custom_torch_ops.md` | General Torch custom-op prototypes should not define the core runtime when native sparse operators are the Cellerator ownership point. | Sparse operator layer in `src/compute/autograd/` or prototype area | `legacy-prototype` | Some model tests still cover old libtorch integration points. | Keep only minimal Torch extension boundaries; move reusable sparse math into framework-independent compute/autograd. |
