...
int sync_fleet(cpre::preprocess_fleet_workspace *fleet) {
    for (unsigned int i = 0u; i < fleet->slot_count; ++i) {
        ...
        if (!check_cuda(cudaSetDevice(::cellerator::runtime::fleet_device_id(fleet->fleet, slot)), "cudaSetDevice sync fleet")) return 0;
        if (!check_cuda(cudaStreamSynchronize(::cellerator::runtime::fleet_stream(fleet->fleet, slot)), "cudaStreamSynchronize fleet")) return 0;
    }
    return 1;
}
...
int main() {
    ...
    const std::uint32_t masks[4] = {
        cpre::qc_group_bit(cpre::qc_group_mt),
        cpre::qc_group_bit(cpre::qc_group_ribo),
        cpre::qc_group_bit(cpre::qc_group_hb),
        cpre::qc_group_bit(cpre::qc_group_mt) | cpre::qc_group_bit(3u)
    };
    const char *group_names[4] = {"mt", "ribo", "hb", "custom"};
    cpre::qc_group_config_view groups{4u, group_names, masks, nullptr};
    cpre::cell_qc_filter_params filter{1.0f, 1u, 0.80f, cpre::qc_group_mt};
    ...
    cpre::preprocess_workspace single_ws;
    cpre::init(&single_ws);
    if (!cpre::setup(&single_ws, 0)) return 3;
    cpre::part_preprocess_result single{};
    if (!cpre::preprocess_blocked_ell_qc_groups_inplace(&full_view, &single_ws, &groups, &filter, 10000.0f, &single)) return 4;
...

    {
        ...
        int device_ids[1] = {0};
        cpre::preprocess_fleet_config config{device_ids, 1u, 1u, cudaStreamNonBlocking, nullptr};
        cpre::preprocess_fleet_workspace fleet;
        cpre::init(&fleet);
        if (!cpre::setup_fleet(&fleet, &config)) return 9;
        cpre::preprocess_fleet_result result{};
        if (!cpre::preprocess_blocked_ell_qc_groups_fleet_inplace(&fleet_view, &fleet, &groups, &filter, 10000.0f, &result)) return 10;
    ...
    if (device_count >= 2) {
        ...
        cpre::preprocess_fleet_config config{device_ids, 2u, 1u, cudaStreamNonBlocking, nullptr};
        cpre::preprocess_fleet_workspace fleet;
        cpre::init(&fleet);
        if (!cpre::setup_fleet(&fleet, &config)) return 17;
        cpre::preprocess_fleet_result result{};
        if (!cpre::preprocess_blocked_ell_qc_groups_fleet_inplace(parts, &fleet, &groups, &filter, 10000.0f, &result)) return 18;
