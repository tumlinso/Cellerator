...
int main() {
    ...
    cpre::init(&workspace);
    if (!cpre::setup(&workspace, 0)) return 1;
    unsigned char flags[4] = {cpre::gene_flag_mito, 0u, 0u, 0u};
    if (!cpre::upload_gene_flags(&workspace, 4u, flags)) return 1;

    cpre::cell_filter_params filter{1.0f, 1u, 0.90f};
    cpre::cell_metrics_view metrics{};
    if (!cpre::compute_cell_metrics(&view, &workspace, &filter, &metrics)) return 1;
