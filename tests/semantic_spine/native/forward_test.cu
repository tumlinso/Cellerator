    ...
    for(unsigned generation=1;generation<=2;++generation){
        ...
        for(auto direction:{ce::orientation::forward}){
            ...
            auto reject=[&](const ce::operation_descriptor& op,const ce::device_state_view& iv,const ce::device_result_view& ov,ex::value_generation gen,cudaStream_t ss) {
                ...
                assert(!ce::enqueue(*p,op,iv,ov,gen,ss));gpu(cudaStreamSynchronize(stream));
                if(outputs){gpu(cudaMemcpy(y.data(),dy,outputs*4,cudaMemcpyDeviceToHost));for(auto v:y)assert(v==123.0f);}
                assert(before==p->report.accepted_forward_launches);
            };
            ...
            assert(p->report.accepted_forward_launches==generation);
