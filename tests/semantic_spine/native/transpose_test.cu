...
static void fixture(unsigned rows,unsigned cols,const std::vector<unsigned>& offsets,const std::vector<unsigned>& sources){
    ...
    for(unsigned generation=1;generation<=2;++generation){
        ...
        assert(std::abs(adjoint_forward-adjoint_transpose)<=1e-5+1e-5*std::abs(adjoint_forward));
        }
        assert(p->report.accepted_forward_launches==generation*3);
        assert(p->report.accepted_transpose_launches==generation*3);
