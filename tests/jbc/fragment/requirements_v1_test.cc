...
int main() {
    ...
    const joint::canonical_interval_v1 interval{0u, 2u};
    ...
    const execution::order_id external_orders[] = {{1u, 1u}, {2u, 1u}};
    const joint::atom_binding_request_v1 binding{
    ...
    const fragment::atom_fragment_query_limits_v1 limits{
        2u, 3u, 4u, 2u, 1u, 5u, 64u};
    fragment::atom_fragment_requirements_v1 requirements{};
    assert(fragment::query_atom_fragment_requirements_v1(
        request, limits, &requirements));
    assert(requirements.local_index_component_count == 1u);
    assert(requirements.local_indexes.bytes == 8u);
    assert(requirements.projection_capacity == 2u);
    assert(requirements.candidate_capacity == 4u);
    assert(requirements.binding_capacity == 2u);
    assert(requirements.transient.bytes == 64u);

    auto malformed_limits = limits;
    malformed_limits.candidate_capacity = 0u;
    assert(fragment::query_atom_fragment_requirements_v1(
               request, malformed_limits, &requirements)
               .code
        == fragment::atom_fragment_requirements_status_code_v1::invalid_limits);

    auto malformed_request = request;
    malformed_request.target_profile = {};
    assert(fragment::query_atom_fragment_requirements_v1(
               malformed_request, limits, &requirements)
               .code
        == fragment::atom_fragment_requirements_status_code_v1::invalid_request);

    auto overflow_limits = limits;
    overflow_limits.projection_capacity =
        std::numeric_limits<std::uint64_t>::max();
    assert(fragment::query_atom_fragment_requirements_v1(
               request, overflow_limits, &requirements)
               .code
        == fragment::atom_fragment_requirements_status_code_v1::
            arithmetic_overflow);
    return 0;
}
