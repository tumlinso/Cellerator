...
int main() {
    ...
    auto invalid = value;
    invalid.requires_partial_algebra = true;
    auto status = decomposition::
        validate_support_embedding_decomposition_v1(invalid);
    assert(status.code == decomposition::
        support_embedding_validation_code_v1::invalid_partial_result_contract);

    const decomposition::dense_width_interval_v1 gap[] = {
        {0u, 4u}, {5u, 11u}};
    invalid = value;
    invalid.embedding_intervals = gap;
    status = decomposition::validate_support_embedding_decomposition_v1(
        invalid);
    assert(status.code == decomposition::
        support_embedding_validation_code_v1::interval_offset_mismatch);
    ...
    status = decomposition::validate_support_embedding_decomposition_v1(
        invalid);
    assert(status.code == decomposition::
        support_embedding_validation_code_v1::unsupported_operation);
    return 0;
}
