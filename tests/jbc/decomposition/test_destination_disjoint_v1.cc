...
int main() {
    ...
    const decomposition::destination_interval_v1 intervals[] = {
        {0u, 3u}, {3u, 4u}, {7u, 3u}};
    decomposition::destination_disjoint_relation_apply_v1 value{};
    value.decomposition_identity = {60u, 61u};
    value.problem = &operation_problem;
    value.destination_extent = 10u;
    value.intervals = intervals;
    value.interval_count = 3u;
    assert(decomposition::validate_destination_disjoint_relation_apply_v1(value));

    auto invalid = value;
    const decomposition::destination_interval_v1 gap[] = {
        {0u, 3u}, {4u, 6u}};
    invalid.intervals = gap;
    invalid.interval_count = 2u;
    auto status =
        decomposition::validate_destination_disjoint_relation_apply_v1(invalid);
    assert(status.code == decomposition::
        destination_disjoint_validation_code_v1::interval_offset_mismatch);
    assert(status.interval_index == 1u);

    const decomposition::destination_interval_v1 overlap[] = {
        {0u, 6u}, {5u, 5u}};
    invalid.intervals = overlap;
    status = decomposition::validate_destination_disjoint_relation_apply_v1(
        invalid);
    assert(status.code == decomposition::
        destination_disjoint_validation_code_v1::interval_offset_mismatch);

    const decomposition::destination_interval_v1 incomplete[] = {{0u, 9u}};
    invalid.intervals = incomplete;
    invalid.interval_count = 1u;
    status = decomposition::validate_destination_disjoint_relation_apply_v1(
        invalid);
    assert(status.code == decomposition::
        destination_disjoint_validation_code_v1::incomplete_partition);

    operation_problem.orientation = operation::relation_orientation::transpose;
    invalid = value;
    status = decomposition::validate_destination_disjoint_relation_apply_v1(
        invalid);
    assert(status.code == decomposition::
        destination_disjoint_validation_code_v1::unsupported_operation);
    return 0;
}
