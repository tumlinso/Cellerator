...
operation::operation_problem problem(operation::typed_relation &relation) {
    ...
    result.kind = operation::operation_kind::relation_apply_transpose;
    result.orientation = operation::relation_orientation::transpose;
    result.persistent_problem_identity = {50u, 51u};
    result.operation_identity = {52u, 53u};
    result.relations = {&relation, 1u};
    result.values_axis = destination;
    result.result_axis = source;
    ...
    result.output.produced_axis = source;
    result.output.canonical_axis = source;
...
int main() {
    ...
    const decomposition::destination_interval_v1 intervals[] = {
        {0u, 4u}, {4u, 6u}};
    decomposition::transpose_source_partials_v1 value{};
    value.decomposition_identity = {60u, 61u};
    value.problem = &operation_problem;
    value.destination_k_extent = 10u;
    value.destination_k_intervals = intervals;
    value.destination_k_interval_count = 2u;
    assert(decomposition::validate_transpose_source_partials_v1(value));

    auto invalid = value;
    invalid.produces_source_partials = false;
    auto status = decomposition::validate_transpose_source_partials_v1(invalid);
    assert(status.code == decomposition::
        transpose_source_partials_validation_code_v1::
            invalid_partial_result_contract);

    const decomposition::destination_interval_v1 overlap[] = {
        {0u, 6u}, {5u, 5u}};
    invalid = value;
    invalid.destination_k_intervals = overlap;
    status = decomposition::validate_transpose_source_partials_v1(invalid);
    assert(status.code == decomposition::
        transpose_source_partials_validation_code_v1::
            interval_offset_mismatch);

    operation_problem.orientation = operation::relation_orientation::forward;
    invalid = value;
    status = decomposition::validate_transpose_source_partials_v1(invalid);
    assert(status.code == decomposition::
        transpose_source_partials_validation_code_v1::unsupported_operation);
    return 0;
}
