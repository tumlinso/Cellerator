...
int main() {
    ...
    const decomposition::dense_width_interval_v1 intervals[] = {
        {0u, 4u}, {4u, 12u}};
    decomposition::dense_width_relation_apply_v1 value{};
    value.decomposition_identity = {60u, 61u};
    value.problem = &operation_problem;
    value.intervals = intervals;
    value.interval_count = 2u;
    assert(decomposition::validate_dense_width_relation_apply_v1(value));

    auto invalid = value;
    invalid.produces_partial_results = true;
    auto status =
        decomposition::validate_dense_width_relation_apply_v1(invalid);
    assert(status.code == decomposition::
        dense_width_validation_code_v1::invalid_partial_result_contract);

    const decomposition::dense_width_interval_v1 gap[] = {
        {0u, 4u}, {5u, 11u}};
    invalid = value;
    invalid.intervals = gap;
    status = decomposition::validate_dense_width_relation_apply_v1(invalid);
    assert(status.code == decomposition::
        dense_width_validation_code_v1::interval_offset_mismatch);

    const decomposition::dense_width_interval_v1 incomplete[] = {{0u, 15u}};
    invalid.intervals = incomplete;
    invalid.interval_count = 1u;
    status = decomposition::validate_dense_width_relation_apply_v1(invalid);
    assert(status.code == decomposition::
        dense_width_validation_code_v1::incomplete_partition);

    operation_problem.orientation = operation::relation_orientation::transpose;
    invalid = value;
    status = decomposition::validate_dense_width_relation_apply_v1(invalid);
    assert(status.code == decomposition::
        dense_width_validation_code_v1::unsupported_operation);
    return 0;
}
