...
int main() {
    ...
    const decomposition::destination_interval_v1 intervals[] = {
        {0u, 4u}, {4u, 6u}};
    decomposition::source_k_relation_apply_v1 value{};
    value.decomposition_identity = {60u, 61u};
    value.problem = &operation_problem;
    value.source_extent = 10u;
    value.source_intervals = intervals;
    value.source_interval_count = 2u;
    assert(decomposition::validate_source_k_relation_apply_v1(value));

    auto invalid = value;
    invalid.requires_partial_algebra = false;
    auto status = decomposition::validate_source_k_relation_apply_v1(invalid);
    assert(status.code == decomposition::
        source_k_validation_code_v1::invalid_partial_result_contract);

    const decomposition::destination_interval_v1 overlap[] = {
        {0u, 6u}, {5u, 5u}};
    invalid = value;
    invalid.source_intervals = overlap;
    status = decomposition::validate_source_k_relation_apply_v1(invalid);
    assert(status.code == decomposition::
        source_k_validation_code_v1::interval_offset_mismatch);

    const decomposition::destination_interval_v1 incomplete[] = {{0u, 9u}};
    invalid.source_intervals = incomplete;
    invalid.source_interval_count = 1u;
    status = decomposition::validate_source_k_relation_apply_v1(invalid);
    assert(status.code == decomposition::
        source_k_validation_code_v1::incomplete_partition);

    operation_problem.kind = operation::operation_kind::segment_reduce;
    invalid = value;
    status = decomposition::validate_source_k_relation_apply_v1(invalid);
    assert(status.code == decomposition::
        source_k_validation_code_v1::unsupported_operation);
    return 0;
}
