...
constexpr std::array<const char *, 7> output_labels = {
...
const wb::source_dataset_summary *inspect_selected_dataset(const ui_state &ui) {
    if (ui.inspect_dataset_selection >= ui.dataset.datasets.size()) return nullptr;
    return &ui.dataset.datasets[ui.inspect_dataset_selection];
}

std::size_t inspect_metadata_table_for_dataset(const ui_state &ui, std::size_t dataset_index) {
    for (std::size_t i = 0; i < ui.dataset.embedded_metadata.size(); ++i) {
        if (ui.dataset.embedded_metadata[i].dataset_index == dataset_index) return i;
    }
...
std::size_t inspect_part_count_for_dataset(const ui_state &ui, std::size_t dataset_index) {
    std::size_t count = 0;
    for (const wb::dataset_partition_summary &partition : ui.dataset.partitions) {
...
std::size_t inspect_part_global_index(const ui_state &ui,
    ...
    std::size_t cursor = 0;
    for (std::size_t i = 0; i < ui.dataset.partitions.size(); ++i) {
        if (ui.dataset.partitions[i].dataset_id != dataset_index) continue;
...
std::size_t inspect_shard_count_for_dataset(const ui_state &ui, std::size_t dataset_index) {
    if (dataset_index >= ui.dataset.datasets.size()) return 0;
    const wb::source_dataset_summary &dataset = ui.dataset.datasets[dataset_index];
    std::size_t count = 0;
    for (const wb::dataset_shard_summary &shard : ui.dataset.shards) {
...
std::size_t inspect_shard_global_index(const ui_state &ui,
                                       std::size_t dataset_index,
                                       std::size_t local_index) {
    if (dataset_index >= ui.dataset.datasets.size()) return std::numeric_limits<std::size_t>::max();
    const wb::source_dataset_summary &dataset = ui.dataset.datasets[dataset_index];
    std::size_t cursor = 0;
    for (std::size_t i = 0; i < ui.dataset.shards.size(); ++i) {
        if (!ranges_overlap(dataset.row_begin, dataset.row_end, ui.dataset.shards[i].row_begin, ui.dataset.shards[i].row_end)) continue;
        if (cursor == local_index) return i;
        ++cursor;
    }
    return std::numeric_limits<std::size_t>::max();
...
void ensure_inspect_metadata_loaded(ui_state *ui) {
    if (ui == nullptr || !ui->dataset.ok) return;
    if (ui->inspect_dataset_selection >= ui->dataset.datasets.size()) {
        clear_inspect_metadata_cache(ui);
        return;
    }
    ...
    ui->inspect_metadata_table = wb::load_embedded_metadata_table(ui->dataset.path, table_index);
...
void clamp_selection(ui_state *ui) {
    ...
    if (!ui->dataset.datasets.empty()) ui->inspect_dataset_selection = std::min(ui->inspect_dataset_selection, ui->dataset.datasets.size() - 1u);
...
void clamp_inspect_viewports(ui_state *ui, const ui_layout &layout) {
    ...
    const std::size_t feature_total = ui->dataset.browse.selected_feature_count;
    ...
    if (ui->inspect_mode == inspect_heatmap_mode::shard_mean) {
        ...
        if (part_index != std::numeric_limits<std::size_t>::max()
            && part_index + 1u < ui->dataset.browse.partition_sample_row_offsets.size()) {
            row_total = (std::size_t) (ui->dataset.browse.partition_sample_row_offsets[part_index + 1u]
                                       - ui->dataset.browse.partition_sample_row_offsets[part_index]);
        }
...
void rebuild_plan(ui_state *ui) {
    if (ui == nullptr) return;
    ui->plan = wb::plan_dataset_ingest(ui->inspection.sources, ui->policy);
    clamp_selection(ui);
    std::ostringstream oss;
...
void open_dataset(ui_state *ui, const std::string &path) {
    if (ui == nullptr) return;
    ui->last_dataset_path = path;
    ui->dataset = wb::summarize_dataset_csh5(path);
    reset_inspect_browser(ui);
    push_log(ui,
             ui->dataset.ok ? log_level::success : log_level::error,
             (ui->dataset.ok ? "opened dataset: " : "failed to inspect dataset: ") + path);
    for (const wb::issue &entry : ui->dataset.issues) push_issue_log(ui, entry);
    if (ui->dataset.ok) ui->active = screen_id::inspect;
}

void run_conversion(ui_state *ui) {
    if (ui == nullptr) return;
    push_log(ui, log_level::note, "starting conversion");
    ui->conversion = wb::convert_plan_to_dataset_csh5(ui->plan);
    ...
    if (ui->conversion.ok) {
        open_dataset(ui, ui->policy.output_path);
...
void run_preprocess(ui_state *ui) {
    if (ui == nullptr) return;
    if (ui->dataset.path.empty()) {
        push_log(ui, log_level::warning, "no dataset.csh5 is open");
        return;
    }
    push_log(ui, log_level::note, "starting preprocess");
    ui->preprocess_run = wb::run_preprocess_pass(ui->dataset.path, ui->preprocess);
...
std::size_t active_count(const ui_state &ui) {
    switch (ui.active) {
        ...
        case screen_id::inspect:
            switch (ui.inspect_focus) {
                case inspect_focus_id::datasets: return ui.dataset.datasets.size();
                case inspect_focus_id::metadata: return ui.inspect_metadata_table.column_names.size();
                case inspect_focus_id::heatmap:
                    switch (ui.inspect_mode) {
                        case inspect_heatmap_mode::dataset_mean: return ui.dataset.datasets.empty() ? 0u : 1u;
                        ...
                        case inspect_heatmap_mode::part_samples: {
                            ...
                            if (part_index == std::numeric_limits<std::size_t>::max()
                                || part_index + 1u >= ui.dataset.browse.partition_sample_row_offsets.size()) return 0u;
                            return (std::size_t) (ui.dataset.browse.partition_sample_row_offsets[part_index + 1u]
                                                  - ui.dataset.browse.partition_sample_row_offsets[part_index]);
                        }
                    }
                    break;
    ...
    switch (ui->active) {
        case screen_id::home:
            switch (ui->home_selection) {
                ...
                case 2: {
                    const std::string path = prompt_string(runtime, layout, "dataset path", ui->last_dataset_path);
                    if (!path.empty()) open_dataset(ui, path);
                    break;
                }
            }
            break;
        ...
        case screen_id::output:
            switch (ui->output_field) {
                ...
                case 2: ui->policy.convert_window_bytes = prompt_u64(runtime, layout, "convert_window_bytes", ui->policy.convert_window_bytes); break;
                case 3: ui->policy.target_shard_bytes = prompt_u64(runtime, layout, "target_shard_bytes", ui->policy.target_shard_bytes); break;
                case 4: ui->policy.reader_bytes = static_cast<std::size_t>(prompt_u64(runtime, layout, "reader_bytes", static_cast<unsigned long>(ui->policy.reader_bytes))); break;
                case 5: ui->policy.cache_dir = prompt_string(runtime, layout, "cache_dir", ui->policy.cache_dir); break;
                case 6: ui->policy.verify_after_write = !ui->policy.verify_after_write; break;
            }
            rebuild_plan(ui);
            break;
        case screen_id::preprocess:
...
void cycle_source_format(ui_state *ui) {
    ...
    switch (source.format) {
        case 0: source.format = cellerator::ingest::dataset::source_mtx; break;
        case cellerator::ingest::dataset::source_mtx: source.format = cellerator::ingest::dataset::source_tenx_mtx; break;
        default: source.format = cellerator::ingest::dataset::source_unknown; break;
    }
    rebuild_plan(ui);
}
...
void move_active_selection(ui_state *ui, int delta, int visible_rows) {
    ...
    if (ui->active == screen_id::inspect) {
        switch (ui->inspect_focus) {
            case inspect_focus_id::datasets: {
                const std::size_t total = ui->dataset.datasets.size();
                if (total == 0u) return;
...
void render_nav(WINDOW *win, const ui_runtime &runtime, const ui_state &ui) {
    ...
    draw_hint(win, runtime, start + 2, "o open dataset");
...
void render_summary(WINDOW *win, const ui_runtime &runtime, const ui_state &ui) {
    ...
    if (ui.active == screen_id::home) {
        ...
        draw_clipped(win, 4, 2, getmaxx(win) - 4, "Open dataset: " + (ui.dataset.ok ? ui.dataset.path : std::string("<none>")));
    ...
    render_summary_status(win, runtime, 3, "Dataset", ui.dataset.ok);
    ...
    if (getmaxy(win) > 10) {
        const std::string dataset_line =
            ui.dataset.ok
                ? ("Dataset rows=" + std::to_string(ui.dataset.rows) + " partitions=" + std::to_string(ui.dataset.num_partitions))
                : "Dataset not open";
        draw_clipped(win, 9, 2, getmaxx(win) - 4, dataset_line);
    }
    if (getmaxy(win) > 11) {
        const std::string browse_line =
            ui.dataset.ok && ui.dataset.browse.available
                ? ("Browse cache ready: features=" + std::to_string(ui.dataset.browse.selected_feature_count))
...
void render_home(WINDOW *win, const ui_runtime &runtime, const ui_state &ui) {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (i == 0) {
            ...
            draw_clipped(win, y + 1, 5, card_w - 2, "Open a dataset.csh5 and jump directly into inspect and preprocess.", A_DIM | color_attr(runtime, cp_home));
        }
    }
...
std::string output_value(const ui_state &ui, int index) {
    switch (index) {
        ...
        case 2: return std::to_string(ui.policy.convert_window_bytes);
        case 3: return std::to_string(ui.policy.target_shard_bytes);
        case 4: return std::to_string(ui.policy.reader_bytes);
        case 5: return ui.policy.cache_dir;
        case 6: return bool_name(ui.policy.verify_after_write);
        default: break;
    }
    return std::string();
}
...
void render_run(WINDOW *win, const ui_runtime &runtime, const ui_state &ui) {
    ...
    draw_hint(win, runtime, 1, "Press r to write the current plan to dataset.csh5");
    draw_clipped(win,
                 3,
                 2,
                 getmaxx(win) - 4,
...
void render_inspect(WINDOW *win, const ui_runtime &runtime, const ui_state &ui) {
    ...
    if (!ui.dataset.ok) {
        draw_hint(win, runtime, 1, "Open a dataset.csh5 with o");
        draw_clipped(win, 3, 2, getmaxx(win) - 4, "The inspect view is the browser for metadata and precomputed heatmaps.", A_DIM);
        return;
    }

    draw_clipped(win, 1, 2, getmaxx(win) - 4, "path: " + ui.dataset.path);
    std::ostringstream line2;
    line2 << "rows=" << ui.dataset.rows << " cols=" << ui.dataset.cols << " nnz=" << ui.dataset.nnz
          << "  datasets=" << ui.dataset.num_datasets << " partitions=" << ui.dataset.num_partitions
          << " shards=" << ui.dataset.num_shards;
    ...
    line3 << "focus=" << inspect_focus_name(ui.inspect_focus)
          ...
          << "  browse=" << (ui.dataset.browse.available ? "ready" : "missing")
          << "  metadata_tables=" << ui.dataset.embedded_metadata.size();
    ...
    for (int row = 0; row < dataset_rows
         && ui.inspect_dataset_scroll + (std::size_t) row < ui.dataset.datasets.size(); ++row) {
        const std::size_t index = ui.inspect_dataset_scroll + (std::size_t) row;
        const wb::source_dataset_summary &dataset = ui.dataset.datasets[index];
        std::ostringstream oss;
    ...
    const wb::source_dataset_summary *dataset = inspect_selected_dataset(ui);
    ...
    if (!ui.dataset.browse.available || ui.dataset.browse.selected_feature_count == 0u) {
    ...
    const std::size_t stride = ui.dataset.browse.selected_feature_count;
    ...
    if (ui.inspect_mode == inspect_heatmap_mode::dataset_mean && dataset != nullptr) {
        ...
        if (offset + feature_total <= ui.dataset.browse.dataset_feature_mean.size()) {
            row_labels.push_back("dataset mean");
            row_ptrs.push_back(ui.dataset.browse.dataset_feature_mean.data() + offset);
        }
        ...
        for (std::size_t local = shard_begin;
            ...
            if (offset + feature_total > ui.dataset.browse.shard_feature_mean.size()) continue;
            std::ostringstream label;
            label << (local == ui.inspect_shard_selection ? "> " : "  ")
                  << "s" << ui.dataset.shards[global_index].shard_id;
            row_labels.push_back(label.str());
            row_ptrs.push_back(ui.dataset.browse.shard_feature_mean.data() + offset);
        }
        ...
        if (part_index != std::numeric_limits<std::size_t>::max()
            && part_index + 1u < ui.dataset.browse.partition_sample_row_offsets.size()) {
            const std::size_t row_begin = ui.dataset.browse.partition_sample_row_offsets[part_index];
            const std::size_t row_end = ui.dataset.browse.partition_sample_row_offsets[part_index + 1u];
            std::ostringstream mode_line;
            mode_line << "Partition samples  selected=" << (ui.inspect_part_selection + 1u)
                      << "/" << std::max<std::size_t>(1u, part_total)
                      << "  partition " << ui.dataset.partitions[part_index].partition_id
                      << " row=[" << ui.dataset.partitions[part_index].row_begin
                      << "," << ui.dataset.partitions[part_index].row_end << ")";
            draw_hint(win, runtime, detail_row, mode_line.str());
            for (std::size_t row = row_begin + ui.inspect_heatmap_row_scroll;
                ...
                const std::uint64_t global_row =
                    row < ui.dataset.browse.partition_sample_global_rows.size()
                        ? ui.dataset.browse.partition_sample_global_rows[row]
                ...
                if (value_offset + feature_total <= ui.dataset.browse.partition_sample_values.size()) {
                    row_ptrs.push_back(ui.dataset.browse.partition_sample_values.data() + value_offset);
                } else {
    ...
    for (std::size_t feature = 0; feature < feature_count; ++feature) {
        ...
        if (feature_begin + feature < ui.dataset.browse.selected_feature_names.size()) {
            feature_line << ui.dataset.browse.selected_feature_names[feature_begin + feature];
        } else {
...
void render_preprocess(WINDOW *win, const ui_runtime &runtime, const ui_state &ui) {
    ...
    draw_hint(win, runtime, 1, "Tune QC thresholds, then press p to run on the open dataset");
...
void render_footer(WINDOW *win, const ui_runtime &runtime, const ui_state &ui) {
    ...
    switch (ui.active) {
        ...
        case screen_id::run: hint = "Run uses the current plan and then opens the output dataset."; break;
        ...
        case screen_id::preprocess: hint = "Preprocess runs on the currently open dataset."; break;
    }
...
void handle_action(ui_state *ui, ui_runtime *runtime, const ui_layout &layout, const action &next, bool *running) {
    ...
    switch (next.id) {
            ...
            if (ui->active == screen_id::inspect
                ...
                if (part_index != std::numeric_limits<std::size_t>::max()
                    ...
                    const std::size_t row_total =
                        (std::size_t) (ui->dataset.browse.partition_sample_row_offsets[part_index + 1u]
                                       - ui->dataset.browse.partition_sample_row_offsets[part_index]);
        ...
        case action_id::inspect_pan_columns:
            if (ui->active == screen_id::inspect && ui->dataset.browse.selected_feature_count != 0u) {
                const std::size_t limit = ui->dataset.browse.selected_feature_count - 1u;
...
void dump_series(const wb::dataset_summary &summary) {
    std::printf("dataset ok=%d rows=%llu cols=%llu nnz=%llu datasets=%llu partitions=%llu shards=%llu\n",
                summary.ok ? 1 : 0,
    ...
    for (const wb::source_dataset_summary &dataset : summary.datasets) {
...
int main(int argc, char **argv) {
    try {
        ui_state ui;
        ui.policy.output_path = "dataset.csh5";
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if ((arg == "--manifest" || arg == "-m") && i + 1 < argc) {
                ...
                ui.last_dataset_path = argv[++i];
            } else if (arg == "--dump-plan") {
                dump_plan_only = true;
            } else if (arg == "--dump-dataset") {
                dump_dataset_only = true;
            } else if ((arg == "--output" || arg == "-O") && i + 1 < argc) {
                ui.policy.output_path = argv[++i];
            } else {
                std::fprintf(stderr,
                             "Usage: %s [--manifest path.tsv] [--open path.csh5] [--output path.csh5] [--dump-plan] [--dump-dataset]\n",
                             argv[0]);
                return 2;
            }
        }
        ...
        if (!ui.last_dataset_path.empty()) open_dataset(&ui, ui.last_dataset_path);
        ...
        if (dump_dataset_only) {
            dump_series(ui.dataset);
            return ui.dataset.ok ? 0 : 1;
        }
