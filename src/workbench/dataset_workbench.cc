...
inline unsigned int infer_format(const source_entry &source) {
    ...
    if (source.matrix_path.find(".h5ad") != std::string::npos) return cseries::source_h5ad;
    return cseries::source_unknown;
}

inline bool load_source_barcodes(const source_entry &source,
                                 ccommon::barcode_table *barcodes,
                                 std::vector<issue> *issues,
                                 const std::string &scope) {
    std::string error;
    if (source.format == cseries::source_mtx || source.format == cseries::source_tenx_mtx) {
        if (source.barcode_path.empty()) {
            push_issue(issues, issue_severity::warning, scope, "barcode path is missing");
            return false;
        }
        if (!ccommon::load_lines(source.barcode_path.c_str(), barcodes)) {
    ...
    if (source.format == cseries::source_h5ad) {
        if (!cellerator::ingest::h5ad::load_barcodes(source.matrix_path.c_str(), barcodes, &error)) {
            push_issue(issues, issue_severity::warning, scope, error.empty() ? "failed to read h5ad obs index" : error);
            return false;
        }
        return true;
    }
...
inline bool load_source_features(const source_entry &source,
    ...
    if (source.format == cseries::source_h5ad) {
        if (!cellerator::ingest::h5ad::load_feature_table(source.matrix_path.c_str(),
                                                          ...
                                                          features,
                                                          &error)) {
            push_issue(issues, issue_severity::warning, scope, error.empty() ? "failed to read h5ad var table" : error);
            return false;
        }
        return true;
    }
...
inline bool load_source_metadata(const source_entry &source,
    ...
    if (source.format == cseries::source_mtx || source.format == cseries::source_tenx_mtx) {
        ...
        if (!ccommon::load_tsv(source.metadata_path.c_str(), metadata, 1)) {
    ...
    if (source.format == cseries::source_h5ad) {
        if (!cellerator::ingest::h5ad::load_metadata_table(source.matrix_path.c_str(), metadata, &error)) {
            push_issue(issues, issue_severity::warning, scope, error.empty() ? "failed to read h5ad obs metadata" : error);
            return false;
        }
        return true;
    }
...
struct blocked_ell_candidate_stream {
    ...
    unsigned long current_partition = ULONG_MAX;
...
inline void flush_candidate_row_block(blocked_ell_candidate_stream *stream) {
    if (stream == nullptr || stream->current_partition == ULONG_MAX) return;
    stream->total_unique_blocks[stream->current_partition] += (unsigned long) stream->active_unique;
    stream->ell_width[stream->current_partition] = std::max(stream->ell_width[stream->current_partition], stream->active_unique);
...
inline bool start_candidate_row_block(blocked_ell_candidate_stream *stream,
    ...
    stream->current_partition = partition_index;
...
inline bool accumulate_candidate_entry(blocked_ell_candidate_stream *stream,
    ...
    if (block_col >= stream->marks.size() || partition_index >= stream->total_unique_blocks.size()) return false;
    if (stream->current_partition != partition_index || stream->current_row_block != row_block) {
        if (!start_candidate_row_block(stream, partition_index, row_block)) return false;
    }
...
inline void choose_part_execution_layout(const ingest_policy &policy,
    ...
    execution_format chosen_format = execution_format::blocked_ell;
    unsigned int chosen_block = 0u;
    unsigned int chosen_ell_cols = 0u;
    ...
    for (const blocked_ell_candidate_stream &candidate : candidates) {
        ...
        const unsigned int ell_width_blocks = partition_index < candidate.ell_width.size() ? candidate.ell_width[partition_index] : 0u;
        const double candidate_fill = row_blocks == 0ul || ell_width_blocks == 0u
            ? 1.0
            : (double) candidate.total_unique_blocks[partition_index] / (double) (row_blocks * (unsigned long) ell_width_blocks);
        ...
        if (chosen_block == 0u
            ...
            chosen_ell_cols = ell_width_blocks * candidate.block_size;
    ...
    if (chosen_block == 0u) {
        for (const blocked_ell_candidate_stream &candidate : candidates) {
            ...
            const std::size_t candidate_bytes = cellshard::packed_blocked_ell_bytes(
                ...
                candidate.block_size,
                sizeof(::real::storage_t));
            if (ell_width_blocks == 0u) continue;
            if (chosen_block == 0u
                || candidate_bytes < chosen_blocked_ell_bytes
                || (candidate_bytes == chosen_blocked_ell_bytes && candidate.block_size > chosen_block)) {
                chosen_block = candidate.block_size;
                chosen_ell_cols = ell_width_blocks * candidate.block_size;
                chosen_fill = 0.0;
                chosen_blocked_ell_bytes = candidate_bytes;
    ...
    if (ell_cols != nullptr) *ell_cols = chosen_ell_cols;
    ...
    if (execution_bytes != nullptr) {
        *execution_bytes = chosen_blocked_ell_bytes;
    }
}

inline bool choose_bucketed_blocked_ell_layout(unsigned long rows,
                                               unsigned int block_size,
                                               double fill_ratio,
                                               std::size_t blocked_ell_bytes,
                                               unsigned int *bucket_count,
                                               std::size_t *bucketed_bytes) {
    const unsigned long row_blocks = block_size != 0u
        ? (rows + (unsigned long) block_size - 1ul) / (unsigned long) block_size
        : 0ul;
    unsigned int chosen_bucket_count = 1u;
    std::size_t chosen_bucketed_bytes = blocked_ell_bytes;

    if (bucket_count != nullptr) *bucket_count = 1u;
    if (bucketed_bytes != nullptr) *bucketed_bytes = blocked_ell_bytes;
    if (block_size == 0u || blocked_ell_bytes == 0u || row_blocks < 4ul || fill_ratio >= 0.72) return false;

    chosen_bucket_count = row_blocks >= 16ul ? 4u : 2u;
    chosen_bucket_count = std::min<unsigned int>(chosen_bucket_count, (unsigned int) row_blocks);
    chosen_bucketed_bytes = (std::size_t) (blocked_ell_bytes * (0.62 + 0.28 * fill_ratio));
    chosen_bucketed_bytes += (std::size_t) rows * sizeof(std::uint32_t) * 2u;
    chosen_bucketed_bytes += ((std::size_t) chosen_bucket_count + 1u) * sizeof(std::uint32_t);
    if (chosen_bucket_count <= 1u || chosen_bucketed_bytes >= blocked_ell_bytes) return false;

    if (bucket_count != nullptr) *bucket_count = chosen_bucket_count;
    if (bucketed_bytes != nullptr) *bucketed_bytes = chosen_bucketed_bytes;
    return true;
}
...
inline bool collect_row_sorted_blocked_ell_metrics(const source_entry &source,
    ...
    if (out_candidates == nullptr || row_offsets == nullptr || num_partitions == 0ul) return false;
    ...
    for (std::size_t i = 0; i < candidate_count; ++i) {
        ...
        candidate.total_unique_blocks.assign(num_partitions, 0ul);
        candidate.ell_width.assign(num_partitions, 0u);
    ...
    while ((rc = cscan::next_line(&reader, &line, &line_len)) > 0) {
        ...
        const unsigned long partition_index = cellshard::find_offset_span(row, row_offsets, num_partitions);
        if (partition_index >= num_partitions) {
            cscan::clear(&reader);
            return false;
        }
        const unsigned long local_row = row - row_offsets[partition_index];
        for (blocked_ell_candidate_stream &candidate : candidates) {
            if (!accumulate_candidate_entry(&candidate, partition_index, local_row, col)) {
                cscan::clear(&reader);
                return false;
            }
        }
...
inline bool collect_h5ad_blocked_ell_metrics(const source_entry &source,
                                             const cmtx::header &header,
                                             const unsigned long *row_offsets,
                                             unsigned long num_partitions,
                                             const ingest_policy &policy,
    ...
    cellerator::ingest::h5ad::selected_matrix_info info;
    const std::size_t candidate_count = choose_blocked_ell_candidate_count(policy);
    std::string error;
    const std::uint64_t chunk_elems = std::max<std::uint64_t>(1u, (std::uint64_t) policy.reader_bytes / (std::uint64_t) sizeof(std::uint64_t));
    std::uint64_t chunk_begin = 0u;
    std::uint64_t chunk_end = 0u;
    unsigned long part = 0ul;
    hid_t file = (hid_t) -1;
    hid_t group = (hid_t) -1;
    bool ok = false;

    if (out_candidates == nullptr || row_offsets == nullptr || num_partitions == 0ul || candidate_count == 0u) return false;
    out_candidates->clear();
    if (!cellerator::ingest::h5ad::probe_selected_matrix(source.matrix_path.c_str(), source.matrix_source.c_str(), &info, &error)) return false;
    if (info.rows != header.rows || info.cols != header.cols || info.nnz != header.nnz_loaded) return false;
    file = H5Fopen(source.matrix_path.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT);
    if (file < 0) goto done;
    group = H5Gopen2(file, info.matrix_path.c_str(), H5P_DEFAULT);
    if (group < 0) goto done;
    if (!cellerator::ingest::h5ad::read_matrix_indptr(group, info, &indptr, &error)) goto done;
    ...
    for (std::size_t i = 0; i < candidate_count; ++i) {
        ...
        blocked_ell_candidate_stream candidate;
        candidate.block_size = block_size;
        candidate.marks.assign((std::size_t) col_blocks, 0u);
        candidate.total_unique_blocks.assign((std::size_t) num_partitions, 0ul);
        candidate.ell_width.assign((std::size_t) num_partitions, 0u);
        candidates.push_back(std::move(candidate));
    }

    if (info.csr) {
        for (unsigned long row = 0; row < header.rows; ++row) {
            while (part + 1ul < num_partitions && row >= row_offsets[part + 1ul]) ++part;
            const std::uint64_t row_begin = indptr[(std::size_t) row];
            const std::uint64_t row_end = indptr[(std::size_t) row + 1u];
            const unsigned long local_row = row - row_offsets[part];
            for (std::uint64_t cursor = row_begin; cursor < row_end; ++cursor) {
                if (cursor < chunk_begin || cursor >= chunk_end) {
                    chunk_begin = cursor;
                    chunk_end = std::min<std::uint64_t>(chunk_begin + chunk_elems, indptr.back());
                    if (!cellerator::ingest::h5ad::read_dataset_u64_range(group, "indices", chunk_begin, chunk_end - chunk_begin, &indices)) {
                        goto done;
                    }
                }
                const std::uint64_t col = indices[(std::size_t) (cursor - chunk_begin)];
                for (blocked_ell_candidate_stream &candidate : candidates) {
                    if (!accumulate_candidate_entry(&candidate, part, local_row, (unsigned long) col)) goto done;
                }
            }
        }
    } else {
        for (unsigned long col = 0; col < header.cols; ++col) {
            ...
            for (std::uint64_t cursor = col_begin; cursor < col_end; ++cursor) {
                ...
                if (row >= header.rows) goto done;
                const unsigned long global_part = cellshard::find_offset_span(row, row_offsets, num_partitions);
                if (global_part >= num_partitions) goto done;
                const unsigned long local_row = row - row_offsets[global_part];
                for (blocked_ell_candidate_stream &candidate : candidates) {
                    if (!accumulate_candidate_entry(&candidate, global_part, local_row, col)) goto done;
                }
            }
        }
    }
...
inline source_entry probe_source_entry(const source_entry &input,
    ...
    output.matrix_sparse = false;
    output.matrix_count_like = false;
    output.matrix_encoding.clear();
    ...
    if (output.format == cseries::source_mtx || output.format == cseries::source_tenx_mtx) {
        ...
        output.matrix_sparse = true;
        output.matrix_count_like = true;
        output.matrix_encoding = "matrix_market";
        std::free(row_nnz);
    } else if (output.format == cseries::source_h5ad) {
        cellerator::ingest::h5ad::selected_matrix_info info;
        std::string error;
        if (!cellerator::ingest::h5ad::probe_selected_matrix(output.matrix_path.c_str(),
                                                             output.matrix_source.c_str(),
                                                             &info,
                                                             &error)) {
            push_issue(issues, issue_severity::error, scope, error.empty() ? "failed to probe h5ad source" : error);
            return output;
        }
        output.rows = info.rows;
        output.cols = info.cols;
        output.nnz = info.nnz;
        output.matrix_sparse = info.sparse;
        output.matrix_count_like = info.count_like;
        output.matrix_encoding = info.csr ? "csr_matrix" : (info.csc ? "csc_matrix" : "");
        if (info.processed_like && !output.allow_processed) {
            push_issue(issues, issue_severity::error, scope, "selected h5ad matrix appears processed; set allow_processed to override");
            return output;
        }
        if (info.processed_like) {
            push_issue(issues, issue_severity::warning, scope, "selected h5ad matrix does not look like raw counts");
        }
    } else {
        push_issue(issues, issue_severity::error, scope, "unsupported source format");
        return output;
    }

    {
        ...
        if (load_source_features(output, &features, issues, scope)) {
...

    {
        ...
        if (load_source_barcodes(output, &barcodes, issues, scope)) {
...

    {
        ...
        if (load_source_metadata(output, &metadata, issues, scope)) {
...
inline bool read_attr_f32(hid_t obj, const char *name, float *value) {
    hid_t attr = H5Aopen(obj, name, H5P_DEFAULT);
    const bool ok = attr >= 0 && H5Aread(attr, H5T_NATIVE_FLOAT, value) >= 0;
    if (attr >= 0) H5Aclose(attr);
    return ok;
}

inline bool read_attr_f64(hid_t obj, const char *name, double *value) {
    hid_t attr = H5Aopen(obj, name, H5P_DEFAULT);
    const bool ok = attr >= 0 && H5Aread(attr, H5T_NATIVE_DOUBLE, value) >= 0;
    if (attr >= 0) H5Aclose(attr);
    return ok;
}
...
inline bool attr_exists(hid_t obj, const char *name) {
    return obj >= 0 && name != nullptr && H5Aexists(obj, name) > 0;
}
...
inline unsigned long find_partition_end_for_row(const std::vector<std::uint64_t> &partition_row_offsets,
    ...
    const unsigned long begin = find_partition_index_for_row(partition_row_offsets, row_end);
    if (begin + 1u < partition_row_offsets.size() && partition_row_offsets[begin] < row_end) return begin + 1u;
    return begin;
}

inline bool load_persisted_preprocess_summary_from_group(hid_t group, persisted_preprocess_summary *out) {
    std::uint32_t raw_counts_available = 0u;
    std::uint32_t processed_matrix_available = 0u;
    std::uint32_t normalized_log1p_metrics = 0u;
    std::uint32_t hvg_available = 0u;
    std::uint32_t mark_mito_from_feature_names = 0u;

    if (out == nullptr) return false;
    *out = persisted_preprocess_summary{};
    if (group < 0) return false;

    if (!read_attr_string(group, "assay", &out->assay)
        || !read_attr_string(group, "matrix_orientation", &out->matrix_orientation)
        || !read_attr_string(group, "matrix_state", &out->matrix_state)
        || !read_attr_string(group, "pipeline_scope", &out->pipeline_scope)
        || !read_attr_string(group, "raw_matrix_name", &out->raw_matrix_name)
        || !read_attr_string(group, "active_matrix_name", &out->active_matrix_name)
        || !read_attr_string(group, "feature_namespace", &out->feature_namespace)
        || !read_attr_string(group, "mito_prefix", &out->mito_prefix)
        || !read_attr_u32(group, "raw_counts_available", &raw_counts_available)
        || !read_attr_u32(group, "processed_matrix_available", &processed_matrix_available)
        || !read_attr_u32(group, "normalized_log1p_metrics", &normalized_log1p_metrics)
        || !read_attr_u32(group, "hvg_available", &hvg_available)
        || !read_attr_u32(group, "mark_mito_from_feature_names", &mark_mito_from_feature_names)
        || !read_attr_u64(group, "rows", &out->rows)
        || !read_attr_u32(group, "cols", &out->cols)
        || !read_attr_u64(group, "nnz", &out->nnz)
        || !read_attr_u32(group, "partitions_processed", &out->partitions_processed)
        || !read_attr_u32(group, "mito_feature_count", &out->mito_feature_count)
        || !read_attr_f32(group, "target_sum", &out->target_sum)
        || !read_attr_f32(group, "min_counts", &out->min_counts)
        || !read_attr_u32(group, "min_genes", &out->min_genes)
        || !read_attr_f32(group, "max_mito_fraction", &out->max_mito_fraction)
        || !read_attr_f32(group, "min_gene_sum", &out->min_gene_sum)
        || !read_attr_f32(group, "min_detected_cells", &out->min_detected_cells)
        || !read_attr_f32(group, "min_variance", &out->min_variance)
        || !read_attr_f64(group, "kept_cells", &out->kept_cells)
        || !read_attr_u32(group, "kept_genes", &out->kept_genes)
        || !read_attr_f64(group, "gene_sum_checksum", &out->gene_sum_checksum)) {
        return false;
    }

    out->raw_counts_available = raw_counts_available != 0u;
    out->processed_matrix_available = processed_matrix_available != 0u;
    out->normalized_log1p_metrics = normalized_log1p_metrics != 0u;
    out->hvg_available = hvg_available != 0u;
    out->mark_mito_from_feature_names = mark_mito_from_feature_names != 0u;
    out->available = true;
    return true;
}
...
builder_path_role infer_builder_path_role(const std::string &path) {
    ...
    if (name.find(".h5ad") != std::string::npos) return builder_path_role::matrix;
...
std::vector<source_entry> sources_from_dataset_drafts(const std::vector<draft_dataset> &drafts) {
    ...
    for (std::size_t i = 0; i < drafts.size(); ++i) {
        ...
        source.matrix_source = draft.matrix_source;
        source.allow_processed = draft.allow_processed;
...
bool export_manifest_tsv(const std::string &path,
    ...
    const char *header = "dataset\tpath\tformat\tfeatures\tbarcodes\tmetadata\tmatrix_source\tallow_processed\trows\tcols\tnnz\n";
    ...
    for (const source_entry &source : inspection.sources) {
        if (std::fprintf(fp,
                         ...
                         tsv_clean(source.matrix_source).c_str(),
...
manifest_inspection inspect_manifest_tsv(const std::string &manifest_path, std::size_t reader_bytes) {
    ...
    for (unsigned int i = 0; i < manifest.count; ++i) {
        ...
        source.matrix_source = ccommon::get(&manifest.matrix_sources, i) != nullptr ? ccommon::get(&manifest.matrix_sources, i) : "x";
        source.allow_processed = manifest.allow_processed != nullptr ? manifest.allow_processed[i] != 0u : false;
...
ingest_plan plan_dataset_ingest(const std::vector<source_entry> &sources, const ingest_policy &policy) {
    ingest_plan plan;
    std::vector<unsigned long> partition_rows;
    std::vector<unsigned long> partition_nnz;
    std::vector<unsigned long> partition_bytes;
    ...
    for (std::size_t source_index = 0; source_index < sources.size(); ++source_index) {
        ...
        unsigned long num_partitions = 0;
        ...
        } else if (source.format == cseries::source_h5ad) {
            std::string error;
            if (!cellerator::ingest::h5ad::scan_row_nnz(source.matrix_path.c_str(),
                                                        source.matrix_source.c_str(),
                                                        &header,
                                                        &row_nnz,
                                                        &error)) {
                push_issue(&plan.issues,
                           issue_severity::error,
                           source.dataset_id,
                           error.empty() ? "failed to scan h5ad row counts during plan generation" : error);
                continue;
            }
        } else {
            push_issue(&plan.issues, issue_severity::error, source.dataset_id, "unsupported source format in ingest planner");
            continue;
        }
        if (!cmtx::plan_row_partitions_by_nnz(row_nnz, header.rows, policy.max_part_nnz, &row_offsets, &num_partitions)
            || !cmtx::build_part_nnz_from_row_nnz(row_nnz, row_offsets, num_partitions, &part_nnz_raw)) {
...

        {
            ccommon::feature_table features;
            ccommon::init(&features);
            if (load_source_features(source, &features, &plan.issues, source.dataset_id)) {
        ...
        const bool have_blocked_ell_metrics =
            source.format == cseries::source_h5ad
                ? collect_h5ad_blocked_ell_metrics(source, header, row_offsets, num_partitions, policy, &blocked_candidates)
                : collect_row_sorted_blocked_ell_metrics(source, header, row_offsets, num_partitions, policy, &blocked_candidates);

        for (unsigned long local_part = 0; local_part < num_partitions; ++local_part) {
            ...
            std::size_t bucketed_blocked_ell_bytes = 0u;
            ...
            if (!have_blocked_ell_metrics) {
                push_issue(&plan.issues,
                           issue_severity::error,
                           source.dataset_id,
                           "failed to derive blocked-ell planning metrics for one or more partitions");
                continue;
            }
            choose_part_execution_layout(
                ...
                &blocked_ell_ell_cols,
                &blocked_ell_fill_ratio,
                &blocked_ell_bytes,
                &execution_bytes);
            if (blocked_ell_block_size == 0u || blocked_ell_ell_cols == 0u || blocked_ell_bytes == 0u || execution_bytes == 0u) {
                push_issue(&plan.issues,
                           issue_severity::error,
                           source.dataset_id,
                           "failed to choose a blocked-ell execution layout for a partition");
                continue;
            }
            part.partition_id = (unsigned long) plan.parts.size();
            ...
            part.rows = rows;
            part.nnz = nnz;
            part.estimated_bytes = blocked_ell_bytes;
            part.execution_bytes = execution_bytes;
            part.blocked_ell_bytes = blocked_ell_bytes;
            part.bucketed_blocked_ell_bytes = blocked_ell_bytes;
            part.blocked_ell_fill_ratio = blocked_ell_fill_ratio;
            part.blocked_ell_block_size = blocked_ell_block_size;
            part.blocked_ell_ell_cols = blocked_ell_ell_cols;
            part.preferred_format = preferred_format;
            if (choose_bucketed_blocked_ell_layout(rows,
                                                   blocked_ell_block_size,
                                                   blocked_ell_fill_ratio,
                                                   blocked_ell_bytes,
                                                   &blocked_ell_bucket_count,
                                                   &bucketed_blocked_ell_bytes)) {
                part.blocked_ell_bucket_count = blocked_ell_bucket_count;
                part.bucketed_blocked_ell_bytes = bucketed_blocked_ell_bytes;
                part.preferred_format = execution_format::bucketed_blocked_ell;
                part.execution_bytes = bucketed_blocked_ell_bytes;
            }
            plan.parts.push_back(part);
            partition_rows.push_back(rows);
            partition_nnz.push_back(nnz);
            partition_bytes.push_back((unsigned long) plan.parts.back().execution_bytes);
            plan.total_estimated_bytes += plan.parts.back().execution_bytes;
        }
        ...
        dataset.partition_begin = global_part_begin;
        dataset.partition_count = num_partitions;
    ...
    if (!partition_rows.empty()) {
        std::vector<unsigned long> partition_aux;
        partition_aux.reserve(plan.parts.size());
        for (const planned_part &part : plan.parts) {
            partition_aux.push_back((unsigned long) cellshard::sparse::pack_blocked_ell_aux(part.blocked_ell_block_size,
                                                                                            part.blocked_ell_block_size != 0u
                                                                                                ? (unsigned long) (part.blocked_ell_ell_cols / part.blocked_ell_block_size)
                                                                                                : 0ul));
        }
        if (!cseries::build_blocked_ell_shards(&shard_plan,
                                               partition_rows.data(),
                                               partition_nnz.data(),
                                               partition_aux.data(),
                                               partition_bytes.data(),
                                               (unsigned long) partition_rows.size(),
                                               policy.target_shard_bytes)) {
            ...
            for (unsigned long shard_id = 0; shard_id < shard_plan.count; ++shard_id) {
                ...
                shard.partition_begin = range.part_begin;
                shard.partition_end = range.part_end;
                ...
                for (unsigned long partition_id = range.part_begin; partition_id < range.part_end && partition_id < plan.parts.size(); ++partition_id) {
                    plan.parts[partition_id].shard_id = shard_id;
                    execution_bytes += plan.parts[partition_id].execution_bytes;
                    blocked_ell_bytes += plan.parts[partition_id].blocked_ell_bytes;
                    bucketed_blocked_ell_bytes += plan.parts[partition_id].bucketed_blocked_ell_bytes;
                    fill_weight += (std::size_t) plan.parts[partition_id].rows;
                    fill_weighted_sum += plan.parts[partition_id].blocked_ell_fill_ratio * (double) plan.parts[partition_id].rows;
                    if (plan.parts[partition_id].preferred_format == execution_format::bucketed_blocked_ell) {
                        ++bucketed_partition_count;
                    }
                    bucketed_segment_count += std::max<std::uint32_t>(1u, plan.parts[partition_id].blocked_ell_bucket_count);
                    if (shard_format == execution_format::unknown) {
                        shard_format = plan.parts[partition_id].preferred_format;
                    } else if (shard_format != plan.parts[partition_id].preferred_format) {
                        shard_format = execution_format::mixed;
                    }
                    if (shard_block_size == 0u) {
                        shard_block_size = plan.parts[partition_id].blocked_ell_block_size;
                    } else if (shard_block_size != plan.parts[partition_id].blocked_ell_block_size) {
                        shard_block_size = 0u;
                    }
                }
                plan.shards.back().execution_bytes = execution_bytes;
                plan.shards.back().blocked_ell_bytes = blocked_ell_bytes;
                plan.shards.back().bucketed_blocked_ell_bytes = bucketed_blocked_ell_bytes;
                ...
                plan.shards.back().bucketed_partition_count = bucketed_partition_count;
                plan.shards.back().bucketed_segment_count = bucketed_segment_count;
                plan.shards.back().preferred_format = shard_format == execution_format::unknown
                    ? execution_format::blocked_ell
                    : shard_format;
            }
        }
    }
...
dataset_summary summarize_dataset_csh5(const std::string &path) {
    ...
    hid_t preprocess = (hid_t) -1;
    hid_t execution = (hid_t) -1;
    hid_t runtime_service = (hid_t) -1;
    ...
    std::vector<std::uint32_t> partition_execution_formats;
    std::vector<std::uint32_t> part_block_sizes;
    std::vector<std::uint32_t> partition_bucket_counts;
    ...
    summary.path = path;
    if (path.empty()) {
        push_issue(&summary.issues, issue_severity::error, "inspect", "dataset.csh5 path is empty");
        return summary;
    }

    file = H5Fopen(path.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT);
    if (file < 0) {
        push_issue(&summary.issues, issue_severity::error, "inspect", "failed to open dataset.csh5");
        return summary;
    }
    ...
    if (!read_attr_u64(file, "rows", &summary.rows)
        ...
        || !read_attr_u64(file, "num_partitions", &summary.num_partitions)
        || !read_attr_u64(file, "num_shards", &summary.num_shards)
        || !read_attr_u64(file, "num_datasets", &summary.num_datasets)) {
        push_issue(&summary.issues, issue_severity::error, "inspect", "failed to read top-level dataset attributes");
        goto done;
    }
    if (attr_exists(file, "matrix_format") && !read_attr_string(file, "matrix_format", &summary.matrix_format)) {
        push_issue(&summary.issues, issue_severity::warning, "inspect", "failed to read matrix_format");
    }
    if (attr_exists(file, "payload_layout") && !read_attr_string(file, "payload_layout", &summary.payload_layout)) {
        push_issue(&summary.issues, issue_severity::warning, "inspect", "failed to read payload_layout");
    }
    ...
    if (datasets < 0 || matrix < 0 || provenance < 0 || codecs < 0) {
        push_issue(&summary.issues, issue_severity::error, "inspect", "dataset.csh5 is missing one or more required groups");
        goto done;
    }
    ...
    for (std::size_t i = 0; i < dataset_ids.size(); ++i) {
        summary.datasets.push_back(source_dataset_summary{
            dataset_ids[i],
    ...
    for (std::size_t i = 0; i < partition_rows.size(); ++i) {
        summary.partitions.push_back(dataset_partition_summary{
            (std::uint64_t) i,
            i < partition_row_offsets.size() ? partition_row_offsets[i] : 0ull,
            i + 1u < partition_row_offsets.size() ? partition_row_offsets[i + 1u] : 0ull,
            i < partition_rows.size() ? partition_rows[i] : 0ull,
            i < partition_nnz.size() ? partition_nnz[i] : 0ull,
    ...
    for (std::size_t i = 0; i + 1u < shard_offsets.size(); ++i) {
        ...
        const unsigned long partition_begin = find_partition_index_for_row(partition_row_offsets, row_begin);
        const unsigned long partition_end = find_partition_end_for_row(partition_row_offsets, row_end);
        summary.shards.push_back(dataset_shard_summary{
            (std::uint64_t) i,
            partition_begin,
    ...
    if (browse >= 0) {
        ...
        if (!read_attr_u32(browse, "selected_feature_count", &browse_summary.selected_feature_count)
            || !read_attr_u32(browse, "sample_rows_per_partition", &browse_summary.sample_rows_per_partition)) {
            ...
            if (!read_dataset_vector(browse, "selected_feature_indices", H5T_NATIVE_UINT32, &browse_summary.selected_feature_indices)
                ...
                || !read_dataset_vector(browse, "partition_sample_row_offsets", H5T_NATIVE_UINT32, &browse_summary.partition_sample_row_offsets)
                || !read_dataset_vector(browse, "partition_sample_global_rows", H5T_NATIVE_UINT64, &browse_summary.partition_sample_global_rows)
                || !read_dataset_vector(browse, "partition_sample_values", H5T_NATIVE_FLOAT, &browse_summary.partition_sample_values)) {
    ...
    preprocess = open_optional_group(file, "/preprocess");
    if (preprocess >= 0) {
        persisted_preprocess_summary preprocess_summary;
        if (!load_persisted_preprocess_summary_from_group(preprocess, &preprocess_summary)) {
            push_issue(&summary.issues, issue_severity::warning, "inspect", "failed to read persisted preprocess metadata");
        } else {
            summary.preprocess = std::move(preprocess_summary);
        }
    }

    execution = open_optional_group(file, "/execution");
    if (execution >= 0) {
        if (!read_attr_u32(execution, "preferred_base_format", &summary.preferred_base_format)) {
            push_issue(&summary.issues, issue_severity::warning, "inspect", "failed to read execution preferred_base_format");
        }
        if (!read_dataset_vector(execution, "partition_execution_formats", H5T_NATIVE_UINT32, &partition_execution_formats)
            || !read_dataset_vector(execution, "partition_blocked_ell_block_sizes", H5T_NATIVE_UINT32, &part_block_sizes)
            || !read_dataset_vector(execution, "partition_blocked_ell_bucket_counts", H5T_NATIVE_UINT32, &partition_bucket_counts)
            || !read_dataset_vector(execution, "partition_blocked_ell_fill_ratios", H5T_NATIVE_FLOAT, &part_fill_ratios)
            || !read_dataset_vector(execution, "partition_execution_bytes", H5T_NATIVE_UINT64, &partition_execution_bytes)
            || !read_dataset_vector(execution, "partition_blocked_ell_bytes", H5T_NATIVE_UINT64, &partition_blocked_ell_bytes)
            || !read_dataset_vector(execution, "partition_bucketed_blocked_ell_bytes", H5T_NATIVE_UINT64, &partition_bucketed_blocked_ell_bytes)
            ...
            || !read_dataset_vector(execution, "shard_bucketed_partition_counts", H5T_NATIVE_UINT32, &shard_bucketed_partition_counts)
            || !read_dataset_vector(execution, "shard_bucketed_segment_counts", H5T_NATIVE_UINT32, &shard_bucketed_segment_counts)
            ...
            || !read_dataset_vector(execution, "shard_bucketed_blocked_ell_bytes", H5T_NATIVE_UINT64, &shard_bucketed_blocked_ell_bytes)
    ...
    runtime_service = open_optional_group(file, "/runtime_service");
    if (runtime_service >= 0) {
        runtime_service_summary service_summary;
        if (!read_attr_u32(runtime_service, "service_mode", &service_summary.service_mode)
            || !read_attr_u32(runtime_service, "live_write_mode", &service_summary.live_write_mode)
            || !read_attr_u32(runtime_service, "prefer_pack_delivery", &service_summary.prefer_pack_delivery)
            || !read_attr_u32(runtime_service, "remote_pack_delivery", &service_summary.remote_pack_delivery)
            || !read_attr_u32(runtime_service, "single_reader_coordinator", &service_summary.single_reader_coordinator)
            || !read_attr_u32(runtime_service, "maintenance_lock_blocks_overwrite", &service_summary.maintenance_lock_blocks_overwrite)
            || !read_attr_u64(runtime_service, "canonical_generation", &service_summary.canonical_generation)
            || !read_attr_u64(runtime_service, "execution_plan_generation", &service_summary.execution_plan_generation)
            || !read_attr_u64(runtime_service, "pack_generation", &service_summary.pack_generation)
            || !read_attr_u64(runtime_service, "service_epoch", &service_summary.service_epoch)
            || !read_attr_u64(runtime_service, "active_read_generation", &service_summary.active_read_generation)
            || !read_attr_u64(runtime_service, "staged_write_generation", &service_summary.staged_write_generation)) {
            push_issue(&summary.issues, issue_severity::warning, "inspect", "failed to read runtime service metadata");
        } else {
            service_summary.available = true;
            summary.runtime_service = std::move(service_summary);
        }
    }
    ...
    if (runtime_service >= 0) H5Gclose(runtime_service);
...
embedded_metadata_table load_embedded_metadata_table(const std::string &path,
    ...
    if (path.empty()) {
        table_summary.error = "dataset path is empty";
        return table_summary;
    }

    file = H5Fopen(path.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT);
    if (file < 0) {
        table_summary.error = "failed to open dataset file";
        return table_summary;
    }
...
persisted_preprocess_table load_persisted_preprocess_table(const std::string &path) {
    persisted_preprocess_table table;
    hid_t file = (hid_t) -1;
    hid_t preprocess = (hid_t) -1;
    ...
    preprocess = open_group(file, "/preprocess");
    if (preprocess < 0) {
        table.error = "persisted preprocess section is missing";
        goto done;
    }

    if (!load_persisted_preprocess_summary_from_group(preprocess, &table.summary)) {
        table.error = "failed to read persisted preprocess metadata";
        goto done;
    }

    cell_qc = open_group(preprocess, "cell_qc");
    gene_qc = open_group(preprocess, "gene_qc");
    if (cell_qc < 0 || gene_qc < 0) {
        table.error = "persisted preprocess payload groups are missing";
        goto done;
    }

    if (!read_dataset_vector(cell_qc, "total_counts", H5T_NATIVE_FLOAT, &table.cell_total_counts)
        || !read_dataset_vector(cell_qc, "mito_counts", H5T_NATIVE_FLOAT, &table.cell_mito_counts)
        || !read_dataset_vector(cell_qc, "max_counts", H5T_NATIVE_FLOAT, &table.cell_max_counts)
        || !read_dataset_vector(cell_qc, "detected_genes", H5T_NATIVE_UINT32, &table.cell_detected_genes)
        || !read_dataset_vector(cell_qc, "keep", H5T_NATIVE_UINT8, &table.cell_keep)) {
        table.error = "failed to read persisted cell preprocess payload";
        goto done;
    }

    if (!read_dataset_vector(gene_qc, "sum", H5T_NATIVE_FLOAT, &table.gene_sum)
        || !read_dataset_vector(gene_qc, "sq_sum", H5T_NATIVE_FLOAT, &table.gene_sq_sum)
        || !read_dataset_vector(gene_qc, "detected_cells", H5T_NATIVE_FLOAT, &table.gene_detected_cells)
        || !read_dataset_vector(gene_qc, "keep", H5T_NATIVE_UINT8, &table.gene_keep)
        || !read_dataset_vector(gene_qc, "flags", H5T_NATIVE_UINT8, &table.gene_flags)) {
        table.error = "failed to read persisted gene preprocess payload";
        goto done;
    }
