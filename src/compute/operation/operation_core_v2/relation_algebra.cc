...
schema_status validate_relation_algebra_problem(
    ...
    if ((problem.core.kind == operation_kind::relation_apply
        ...
        && (problem.semantic_flags & (alpha_applied_once | beta_applied_once))
            != (alpha_applied_once | beta_applied_once)) {
        return {schema_status_code::invalid_operation, 0};
    }
    if (problem.core.kind == operation_kind::contract_on_support
        && (problem.semantic_flags & stable_logical_edge_output) == 0) {
        return {schema_status_code::invalid_operation, 0};
    }
    ...
    if (problem.core.kind == operation_kind::segment_reduce) {
        const std::uint32_t required = problem.segment == segment_operation::sum
            ? empty_sum_is_zero : empty_max_is_negative_infinity;
        if ((problem.semantic_flags & required) == 0) {
            return {schema_status_code::invalid_operation, 0};
        }
    }
    ...
    if (problem.core.kind == operation_kind::segment_normalize) {
        const std::uint32_t required = empty_normalization_has_no_output
            | singleton_normalization_is_exact;
        if ((problem.semantic_flags & required) != required) {
            return {schema_status_code::invalid_operation, 0};
        }
    }
    if (problem.core.kind == operation_kind::edge_map_or_gate) {
        if (!valid_edge_operation(problem.edge)
            ...
            || (problem.semantic_flags & projection_aware_edge_values) == 0
            || (problem.edge == edge_operation::active_support_mask
                && (problem.semantic_flags & support_superset_preserved) == 0)) {
            return {schema_status_code::invalid_operation, 0};
        }
