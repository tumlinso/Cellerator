...
source_k_validation_result_v1 failure(source_k_validation_code_v1 code,
...
source_k_validation_result_v1 validate_source_k_relation_apply_v1(
    ...
    using code = source_k_validation_code_v1;

    if (decomposition.schema_version != source_k_schema_version_v1)
    ...
    if (decomposition.source_extent == 0u)
        return failure(code::invalid_source_extent);
    if (decomposition.source_interval_count == 0u)
        return failure(code::invalid_interval_count);
    if (decomposition.source_intervals == nullptr)
        return failure(code::missing_intervals);
    if (decomposition.split_axis != split_axis_kind_v1::source
    ...
    for (std::uint64_t index = 0u;
         index < decomposition.source_interval_count; ++index) {
        const auto &interval = decomposition.source_intervals[index];
        ...
        if (expected_begin > decomposition.source_extent)
            return failure(code::interval_range_overflow, index);
    }
    if (expected_begin != decomposition.source_extent)
        return failure(code::incomplete_partition,
            decomposition.source_interval_count - 1u);
    return {};
}
