...
destination_disjoint_validation_result_v1 failure(
...
validate_destination_disjoint_relation_apply_v1(
    ...
    using code = destination_disjoint_validation_code_v1;

    if (decomposition.schema_version
        != destination_disjoint_schema_version_v1)
    ...
    if (decomposition.destination_extent == 0u)
        return failure(code::invalid_destination_extent);
    ...
    for (std::uint64_t index = 0u; index < decomposition.interval_count;
        ...
        if (expected_begin > decomposition.destination_extent)
            return failure(code::interval_range_overflow, index);
    }
    if (expected_begin != decomposition.destination_extent)
