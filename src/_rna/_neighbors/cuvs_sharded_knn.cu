...
static inline int active_device_ids(int gpu_limit, std::vector<int> *out)
{
    int count = 0;
    int i = 0;
    ...
    for (i = 0; i < count; ++i) out->push_back(i);
    return 1;
}
...
static inline int merge_host_candidates(unsigned long rows,
                                        ...
                                        int select_min,
                                        int negate_back,
                                        const std::vector< std::vector<std::int64_t> > &candidate_ids,
                                        const std::vector< std::vector<float> > &candidate_values,
                                        knn_result_host *out)
{
    unsigned long row = 0;
...
#pragma omp parallel for schedule(static)
    ...
    for (row = 0; row < rows; ++row) {
        std::vector<float> best_values((std::size_t) k, worst_value(select_min));
        std::vector<std::int64_t> best_ids((std::size_t) k, (std::int64_t) -1);
        for (int source = 0; source < (int) candidate_ids.size(); ++source) {
            const std::vector<std::int64_t> &ids = candidate_ids[(std::size_t) source];
            const std::vector<float> &values = candidate_values[(std::size_t) source];
            int slot = 0;
        ...
        for (int source = 0; source < k; ++source) {
            const std::size_t off = (std::size_t) row * (std::size_t) k + (std::size_t) source;
            out->neighbors[off] = best_ids[(std::size_t) source];
            out->distances[off] =
                negate_back && isfinite(best_values[(std::size_t) source]) ? -best_values[(std::size_t) source] : best_values[(std::size_t) source];
        }
    }
    return 1;
}
                          ...
                          knn_result_host *result)
{
    ...
    std::vector<device_sparse_csr> index_cache;
    std::vector< std::vector<unsigned long> > shards_by_device;
    ...
    shards_by_device.assign((std::size_t) ctx.device_count, std::vector<unsigned long>());
    for (shard = 0; shard < view->num_shards; ++shard) {
        init(&index_cache[(std::size_t) shard]);
        if (map.device_slot[shard] < 0 || map.device_slot[shard] >= (int) ctx.device_count) goto fail;
        shards_by_device[(std::size_t) map.device_slot[shard]].push_back(shard);
    }

    for (device = 0; device < (int) ctx.device_count; ++device) {
        const std::vector<unsigned long> &owned = shards_by_device[(std::size_t) device];
        for (shard = 0; shard < owned.size(); ++shard) {
            if (!pack_sparse_shard_to_device(view,
                                             storage,
                                             owned[shard],
                                             ctx.device_ids[device],
                                             params.drop_host_parts_after_index_pack,
                                             &index_cache[(std::size_t) owned[shard]])) goto fail;
        }
    }
    ...
    for (shard = 0; shard < view->num_shards; ++shard) {
        ...
        std::vector< std::vector<std::int64_t> > final_ids_by_device((std::size_t) ctx.device_count);
        std::vector< std::vector<float> > final_dist_by_device((std::size_t) ctx.device_count);
        ...
        for (device = 0; device < (int) ctx.device_count; ++device) {
            ...
            std::size_t final_count = (std::size_t) query_rows * (std::size_t) params.k;
            ...
            if (!cuda_ok(cudaMalloc((void **) &d_merge_in_distances,
                                    (std::size_t) query_rows * (std::size_t) local_k * owned.size() * sizeof(float)),
                         "cudaMalloc(d_merge_in_distances)")) goto device_fail;
            if (!cuda_ok(cudaMalloc((void **) &d_merge_in_neighbors,
                                    (std::size_t) query_rows * (std::size_t) local_k * owned.size() * sizeof(std::int64_t)),
            ...
            if (!cuda_ok(cudaMalloc((void **) &d_zero_translations, owned.size() * sizeof(std::int64_t)), "cudaMalloc(d_zero_translations)")) goto device_fail;
            if (!cuda_ok(cudaMemset(d_zero_translations, 0, owned.size() * sizeof(std::int64_t)), "cudaMemset(d_zero_translations)")) goto device_fail;

            for (std::size_t local_shard = 0; local_shard < owned.size(); ++local_shard) {
            ...
            cuvs::neighbors::knn_merge_parts(
                ...
                raft::make_device_matrix_view<const float, std::int64_t, raft::row_major>(
                    ...
                    (std::int64_t) local_k * (std::int64_t) owned.size()),
                ...
                raft::make_device_matrix_view<std::int64_t, std::int64_t, raft::row_major>(
                    ...
                    (std::int64_t) query_rows,
                    (std::int64_t) params.k),
                raft::make_device_vector_view<std::int64_t, std::int64_t>(d_zero_translations, (std::int64_t) owned.size()));

            if (!cuda_ok(cudaStreamSynchronize(ctx.streams[device]), "cudaStreamSynchronize(local merge)")) goto device_fail;
            final_ids_by_device[(std::size_t) device].resize(final_count);
            final_dist_by_device[(std::size_t) device].resize(final_count);
            if (!cuda_ok(cudaMemcpy(final_ids_by_device[(std::size_t) device].data(),
                                    d_final_neighbors,
                                    final_count * sizeof(std::int64_t),
                                    cudaMemcpyDeviceToHost),
                         "cudaMemcpy(final ids -> host)")) goto device_fail;
            if (!cuda_ok(cudaMemcpy(final_dist_by_device[(std::size_t) device].data(),
                                    d_final_distances,
...
device_fail:
...

        {
            ...
            std::size_t base = 0;
            init(&shard_result);
            if (!merge_host_candidates(query_rows, params.k, 1, negate_distance, final_ids_by_device, final_dist_by_device, &shard_result)) goto fail;
                       ...
                       knn_result_host *result)
{
    ...
    {
        raft::device_resources_snmg clique(device_ids);
                               ...
                               knn_result_host *result)
{
    ...
    std::vector<device_dense_exact_index> indices;
    std::vector<__half> packed_host;
    ...
    for (query_begin = 0; query_begin < view->rows; query_begin += (unsigned long) query_block_rows) {
        ...
        std::vector< std::vector<std::int64_t> > final_ids_by_device((std::size_t) ctx.device_count);
        std::vector< std::vector<float> > final_values_by_device((std::size_t) ctx.device_count);
        ...
        for (device = 0; device < (int) ctx.device_count; ++device) {
            ...
            final_ids_by_device[(std::size_t) device].resize((std::size_t) query_rows * (std::size_t) params.k);
            final_values_by_device[(std::size_t) device].resize((std::size_t) query_rows * (std::size_t) params.k);
            if (!cuda_ok(cudaMemcpy(final_ids_by_device[(std::size_t) device].data(),
                                    d_best_indices,
                                    (std::size_t) query_rows * (std::size_t) params.k * sizeof(std::int64_t),
                                    cudaMemcpyDeviceToHost),
                         "cudaMemcpy(best indices -> host)")) goto device_fail;
            if (!cuda_ok(cudaMemcpy(final_values_by_device[(std::size_t) device].data(),
                                    d_best_values,
                                    (std::size_t) query_rows * (std::size_t) params.k * sizeof(float),
                                    cudaMemcpyDeviceToHost),
