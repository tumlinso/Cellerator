...
__host__ __device__ __forceinline__ Real uscsr_get_value(const uniscaled_csr_matrix<Bits, Real>* matrix, int row, int column) {
    ...
    return uscsr_dequantize_code<Bits>(code, matrix->gene_scales[column]);
}
...
pack8_next:
        ...
        *out = static_cast<unsigned char>(uscsr_quantize_code<Bits>(
            ...
            matrix->gene_scales[matrix->colIdx[index]]));
        ++index;
        ++out;
        goto pack8_next;
    }
...
pack4_pair:
        if (index + 1 < end) {
            ...
            q1 = uscsr_quantize_code<Bits>(
                ...
                matrix->gene_scales[matrix->colIdx[index + 1]]);
...
pack2_quad:
        if (index + 3 < end) {
            ...
            q2 = uscsr_quantize_code<Bits>(
                ...
                matrix->gene_scales[matrix->colIdx[index + 2]]);
            q3 = uscsr_quantize_code<Bits>(
                values_by_nnz[index + 3 - value_base],
                matrix->gene_scales[matrix->colIdx[index + 3]]);
