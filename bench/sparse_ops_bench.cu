...
bench_config parse_args(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--mode") {
            ...
            std::cout
                << "Usage: sparseOpsBench [--mode base-spmv|pair-row-spmv|fleet-feature-spmv|base-csr-spmm|base-blocked-ell-spmm] "
...
template<typename T>
runtime::device_buffer<T> allocate_on_device(int device, std::size_t count) {
    runtime::cuda_require(cudaSetDevice(device), "cudaSetDevice(allocate_on_device)");
    return runtime::allocate_device_buffer<T>(count);
}

template<typename T>
void upload_on_device(int device, runtime::device_buffer<T> *dst, const T *src, std::size_t count) {
    runtime::cuda_require(cudaSetDevice(device), "cudaSetDevice(upload_on_device)");
    runtime::upload_device_buffer(dst, src, count);
}
...
void run_base_spmv(const bench_config &cfg) {
    ...
    auto major_ptr = runtime::allocate_device_buffer<std::uint32_t>(static_cast<std::size_t>(cfg.rows) + 1u);
    auto minor_idx = runtime::allocate_device_buffer<std::uint32_t>(matrix.nnz);
    auto values = runtime::allocate_device_buffer<__half>(matrix.nnz);
    auto dense = runtime::allocate_device_buffer<float>(cfg.cols);
    auto out = runtime::allocate_device_buffer<float>(cfg.rows);

    runtime::upload_device_buffer(&major_ptr, matrix.major_ptr.get(), static_cast<std::size_t>(cfg.rows) + 1u);
    runtime::upload_device_buffer(&minor_idx, matrix.minor_idx.get(), matrix.nnz);
    runtime::upload_device_buffer(&values, matrix.values.get(), matrix.nnz);
    runtime::upload_device_buffer(&dense, vector.get(), cfg.cols);
    runtime::cuda_require(cudaStreamSynchronize(ctx.stream), "cudaStreamSynchronize(base setup)");

    for (std::uint32_t i = 0; i < cfg.warmup; ++i) {
        sparse_ops::base::csr_spmv_fwd_f16_f32(ctx, major_ptr.data, minor_idx.data, values.data, cfg.rows, dense.data, out.data);
    }
    runtime::cuda_require(cudaStreamSynchronize(ctx.stream), "cudaStreamSynchronize(base warmup)");

    const auto start = std::chrono::steady_clock::now();
    for (std::uint32_t i = 0; i < cfg.iters; ++i) {
        sparse_ops::base::csr_spmv_fwd_f16_f32(ctx, major_ptr.data, minor_idx.data, values.data, cfg.rows, dense.data, out.data);
    }
    runtime::cuda_require(cudaStreamSynchronize(ctx.stream), "cudaStreamSynchronize(base timed)");
...
void run_base_csr_spmm(const bench_config &cfg) {
    ...
    auto minor_idx = runtime::allocate_device_buffer<std::uint32_t>(matrix.nnz);
    auto values = runtime::allocate_device_buffer<__half>(matrix.nnz);
    auto dense = runtime::allocate_device_buffer<float>(static_cast<std::size_t>(cfg.cols) * cfg.out_cols);
    auto out = runtime::allocate_device_buffer<float>(static_cast<std::size_t>(cfg.rows) * cfg.out_cols);

    runtime::upload_device_buffer(&major_ptr, matrix.major_ptr.get(), static_cast<std::size_t>(cfg.rows) + 1u);
    runtime::upload_device_buffer(&minor_idx, matrix.minor_idx.get(), matrix.nnz);
    runtime::upload_device_buffer(&values, matrix.values.get(), matrix.nnz);
    runtime::upload_device_buffer(&dense, rhs.get(), static_cast<std::size_t>(cfg.cols) * cfg.out_cols);
    runtime::cuda_require(cudaStreamSynchronize(ctx.stream), "cudaStreamSynchronize(csr spmm setup)");

    for (std::uint32_t i = 0; i < cfg.warmup; ++i) {
        sparse_project::csr_spmm_fwd_f16_f32(ctx, major_ptr.data, minor_idx.data, values.data, cfg.rows, cfg.cols, dense.data, cfg.out_cols, cfg.out_cols, out.data, cfg.out_cols);
    }
    runtime::cuda_require(cudaStreamSynchronize(ctx.stream), "cudaStreamSynchronize(csr spmm warmup)");

    const auto start = std::chrono::steady_clock::now();
    for (std::uint32_t i = 0; i < cfg.iters; ++i) {
        sparse_project::csr_spmm_fwd_f16_f32(ctx, major_ptr.data, minor_idx.data, values.data, cfg.rows, cfg.cols, dense.data, cfg.out_cols, cfg.out_cols, out.data, cfg.out_cols);
    }
    runtime::cuda_require(cudaStreamSynchronize(ctx.stream), "cudaStreamSynchronize(csr spmm timed)");
    const auto stop = std::chrono::steady_clock::now();
...
void run_base_blocked_ell_spmm(const bench_config &cfg) {
    ...
    auto block_col_idx = runtime::allocate_device_buffer<std::uint32_t>(static_cast<std::size_t>(cs::sparse::row_block_count(&blocked)) * cs::sparse::ell_width_blocks(&blocked));
    auto blocked_values = runtime::allocate_device_buffer<__half>(static_cast<std::size_t>(blocked.rows) * blocked.ell_cols);
    auto dense = runtime::allocate_device_buffer<__half>(static_cast<std::size_t>(cfg.cols) * cfg.out_cols);
    auto out = runtime::allocate_device_buffer<float>(static_cast<std::size_t>(cfg.rows) * cfg.out_cols);
    runtime::cusparse_cache cache;
    runtime::init(&cache);

    runtime::upload_device_buffer(&block_col_idx, blocked.blockColIdx, static_cast<std::size_t>(cs::sparse::row_block_count(&blocked)) * cs::sparse::ell_width_blocks(&blocked));
    runtime::upload_device_buffer(&blocked_values, blocked.val, static_cast<std::size_t>(blocked.rows) * blocked.ell_cols);
    runtime::upload_device_buffer(&dense, rhs.get(), static_cast<std::size_t>(cfg.cols) * cfg.out_cols);
    runtime::cuda_require(cudaStreamSynchronize(ctx.stream), "cudaStreamSynchronize(blocked ell spmm setup)");
    print_blocked_ell_meta(blocked, elapsed_ms(convert_start, convert_stop));

    for (std::uint32_t i = 0; i < cfg.warmup; ++i) {
        sparse_project::blocked_ell_spmm_fwd_f16_f16_f32_lib(ctx, &cache, blocked.val, block_col_idx.data, blocked_values.data, blocked.rows, blocked.cols, blocked.block_size, blocked.ell_cols, dense.data, cfg.out_cols, cfg.out_cols, out.data, cfg.out_cols);
    }
    runtime::cuda_require(cudaStreamSynchronize(ctx.stream), "cudaStreamSynchronize(blocked ell spmm warmup)");

    const auto start = std::chrono::steady_clock::now();
    for (std::uint32_t i = 0; i < cfg.iters; ++i) {
        sparse_project::blocked_ell_spmm_fwd_f16_f16_f32_lib(ctx, &cache, blocked.val, block_col_idx.data, blocked_values.data, blocked.rows, blocked.cols, blocked.block_size, blocked.ell_cols, dense.data, cfg.out_cols, cfg.out_cols, out.data, cfg.out_cols);
    }
    runtime::cuda_require(cudaStreamSynchronize(ctx.stream), "cudaStreamSynchronize(blocked ell spmm timed)");
    const auto stop = std::chrono::steady_clock::now();
...
void run_pair_row_spmv(const bench_config &cfg) {
    ...
    runtime::init(&fleet);
    runtime::discover_fleet(&fleet, true, cudaStreamNonBlocking, true);
    ...
    const int device0 = runtime::fleet_device_id(fleet, slots[0]);
    const int device1 = runtime::fleet_device_id(fleet, slots[1]);
    ...
    runtime::synchronize_slots(fleet, slots, 2u);
    for (std::uint32_t i = 0; i < cfg.warmup; ++i) {
        sparse_ops::dist::launch_csr_spmv_fwd_f16_f32(&fleet, slots, 2u, major_ptrs, minor_ptrs, value_ptrs, rows, vector_ptrs, out_ptrs);
    }
    runtime::synchronize_slots(fleet, slots, 2u);
...
void run_fleet_feature_spmv(const bench_config &cfg) {
    ...
    require(runtime::default_mode_fleet_slots(fleet, slots, 4u) == 4u, "default fleet slots unavailable");
    ...
    auto major0 = allocate_on_device<std::uint32_t>(runtime::fleet_device_id(fleet, slots[0]), static_cast<std::size_t>(cfg.rows) + 1u);
    auto minor0 = allocate_on_device<std::uint32_t>(runtime::fleet_device_id(fleet, slots[0]), matrices[0].nnz);
    auto values0 = allocate_on_device<__half>(runtime::fleet_device_id(fleet, slots[0]), matrices[0].nnz);
    auto vector0 = allocate_on_device<float>(runtime::fleet_device_id(fleet, slots[0]), cols_per_slot);
    auto out0 = allocate_on_device<float>(runtime::fleet_device_id(fleet, slots[0]), cfg.rows);
    auto reduced0 = allocate_on_device<float>(runtime::fleet_device_id(fleet, slots[0]), cfg.rows);

    auto major1 = allocate_on_device<std::uint32_t>(runtime::fleet_device_id(fleet, slots[1]), static_cast<std::size_t>(cfg.rows) + 1u);
    auto minor1 = allocate_on_device<std::uint32_t>(runtime::fleet_device_id(fleet, slots[1]), matrices[1].nnz);
    auto values1 = allocate_on_device<__half>(runtime::fleet_device_id(fleet, slots[1]), matrices[1].nnz);
    auto vector1 = allocate_on_device<float>(runtime::fleet_device_id(fleet, slots[1]), cols_per_slot);
    auto out1 = allocate_on_device<float>(runtime::fleet_device_id(fleet, slots[1]), cfg.rows);

    auto major2 = allocate_on_device<std::uint32_t>(runtime::fleet_device_id(fleet, slots[2]), static_cast<std::size_t>(cfg.rows) + 1u);
    auto minor2 = allocate_on_device<std::uint32_t>(runtime::fleet_device_id(fleet, slots[2]), matrices[2].nnz);
    auto values2 = allocate_on_device<__half>(runtime::fleet_device_id(fleet, slots[2]), matrices[2].nnz);
    auto vector2 = allocate_on_device<float>(runtime::fleet_device_id(fleet, slots[2]), cols_per_slot);
    auto out2 = allocate_on_device<float>(runtime::fleet_device_id(fleet, slots[2]), cfg.rows);

    auto major3 = allocate_on_device<std::uint32_t>(runtime::fleet_device_id(fleet, slots[3]), static_cast<std::size_t>(cfg.rows) + 1u);
    auto minor3 = allocate_on_device<std::uint32_t>(runtime::fleet_device_id(fleet, slots[3]), matrices[3].nnz);
    auto values3 = allocate_on_device<__half>(runtime::fleet_device_id(fleet, slots[3]), matrices[3].nnz);
    auto vector3 = allocate_on_device<float>(runtime::fleet_device_id(fleet, slots[3]), cols_per_slot);
    auto out3 = allocate_on_device<float>(runtime::fleet_device_id(fleet, slots[3]), cfg.rows);

    upload_on_device(runtime::fleet_device_id(fleet, slots[0]), &major0, matrices[0].major_ptr.get(), static_cast<std::size_t>(cfg.rows) + 1u);
    upload_on_device(runtime::fleet_device_id(fleet, slots[0]), &minor0, matrices[0].minor_idx.get(), matrices[0].nnz);
    upload_on_device(runtime::fleet_device_id(fleet, slots[0]), &values0, matrices[0].values.get(), matrices[0].nnz);
    upload_on_device(runtime::fleet_device_id(fleet, slots[0]), &vector0, vectors0.get(), cols_per_slot);

    upload_on_device(runtime::fleet_device_id(fleet, slots[1]), &major1, matrices[1].major_ptr.get(), static_cast<std::size_t>(cfg.rows) + 1u);
    upload_on_device(runtime::fleet_device_id(fleet, slots[1]), &minor1, matrices[1].minor_idx.get(), matrices[1].nnz);
    upload_on_device(runtime::fleet_device_id(fleet, slots[1]), &values1, matrices[1].values.get(), matrices[1].nnz);
    upload_on_device(runtime::fleet_device_id(fleet, slots[1]), &vector1, vectors1.get(), cols_per_slot);

    upload_on_device(runtime::fleet_device_id(fleet, slots[2]), &major2, matrices[2].major_ptr.get(), static_cast<std::size_t>(cfg.rows) + 1u);
    upload_on_device(runtime::fleet_device_id(fleet, slots[2]), &minor2, matrices[2].minor_idx.get(), matrices[2].nnz);
    upload_on_device(runtime::fleet_device_id(fleet, slots[2]), &values2, matrices[2].values.get(), matrices[2].nnz);
    upload_on_device(runtime::fleet_device_id(fleet, slots[2]), &vector2, vectors2.get(), cols_per_slot);

    upload_on_device(runtime::fleet_device_id(fleet, slots[3]), &major3, matrices[3].major_ptr.get(), static_cast<std::size_t>(cfg.rows) + 1u);
    upload_on_device(runtime::fleet_device_id(fleet, slots[3]), &minor3, matrices[3].minor_idx.get(), matrices[3].nnz);
    upload_on_device(runtime::fleet_device_id(fleet, slots[3]), &values3, matrices[3].values.get(), matrices[3].nnz);
    upload_on_device(runtime::fleet_device_id(fleet, slots[3]), &vector3, vectors3.get(), cols_per_slot);
    ...
    runtime::synchronize_slots(fleet, slots, 4u);
    for (std::uint32_t i = 0; i < cfg.warmup; ++i) {
        sparse_ops::dist::launch_csr_spmv_fwd_f16_f32(&fleet, slots, 4u, major_ptrs, minor_ptrs, value_ptrs, rows, vector_ptrs, out_ptrs);
        sparse_ops::dist::reduce_sum_to_leader_f32(&fleet, slots, 4u, partial_ptrs, cfg.rows, reduced0.data);
    }
    const unsigned int leader_only[1] = { slots[0] };
    runtime::synchronize_slots(fleet, leader_only, 1u);
...
int main(int argc, char **argv) {
    cellerator::bench::benchmark_mutex_guard benchmark_mutex("sparseOpsBench");
