...
static int run_preprocess_benchmark(const config &cfg) {
    ...
    if (!cpre::setup(&workspace, 0, (cudaStream_t) 0)) goto done;
    if (!cpre::reserve(&workspace, (unsigned int) max_part_rows(&built), cfg.cols, (unsigned int) max_part_nnz(&built))) goto done;
        ...
        for (unsigned int iter = 0; iter < cfg.repeats; ++iter) {
...

            {
                ...
                for (unsigned long part = 0; part < built.num_partitions; ++part) {
                    ...
                    if (!cpre::compute_cell_metrics_compressed_fallback(&part_view, &workspace, &cell_filter, &cell)) goto done;
                    if (!cpre::normalize_log1p_compressed_fallback_inplace(&part_view, &workspace, cell.total_counts, cell.keep_cells, cfg.target_sum)) goto done;
                    if (!cpre::accumulate_gene_metrics_compressed_fallback(&part_view, &workspace, cell.keep_cells, nullptr)) goto done;
...

            {
                ...
                if (!cpre::build_gene_filter_mask(&workspace, cfg.cols, &gene_filter, nullptr)) goto done;
...
int main(int argc, char **argv) {
    cpre::bench::benchmark_mutex_guard benchmark_mutex("CelleratorPreprocessScrnaBench");
    config cfg;
